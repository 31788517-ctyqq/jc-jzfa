...
var ODDS_DIR = path.join(__dirname, '..', 'server', 'odds_history');
...
async function syncMatchList(dateStr) {
  ...
  log('[MATCH] Syncing ' + dateStr + ' ' + week);
...
// Sync recommendations for matches
async function syncRecommends(dateStr, matches) {
  log('[RECS] Syncing recommendations for ' + dateStr);
  var token = await tokenMgr.getToken();
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    try {
      ...
      if (recRes.code === 1 && recRes.data && recRes.data.length) {
        // Store in OLD format (t/n/rs) to match existing data schema
...
async function syncOdds(dateStr) {
  log('[ODDS] Fetching odds for ' + dateStr);

  var filePath = path.join(ODDS_DIR, dateStr + '.json');
  if (fs.existsSync(filePath) && fs.statSync(filePath).size > 100) {
    log('[ODDS] ' + dateStr + ' already exists, skip');
    return;
  }
  ...
  try {
    ...
    log('[ODDS] ' + dateStr + ' done: ' + matchNums.length + ' matches');
  } catch(e) {
...
// Main
(async function() {
  log('══════ Batch sync for ' + DATES.length + ' dates ══════');
