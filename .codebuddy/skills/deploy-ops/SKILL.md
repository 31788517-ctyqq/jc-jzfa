...
**修改 CSS/HTML/前端 JS 后，AI 必须先在浏览器中验证效果，再请求部署确认。**
...
> 教训：V12 CSS 变量化时 `6px;` `10px;` 被盲目替换为 radius 变量，
> 导致 font-size/gap/padding 全部变形。如果当时截图验证，一眼就能发现。

**部署是高风险操作，绝对禁止 AI 自行执行。包括 `--files-only` 热修复、`--fast` 全量部署。**

- 部署前必须向用户展示：变更清单 + dry-run 结果 + 回退 tag
- 必须等到用户明确回复"确认"/"执行"/"部署"后才能执行
- 即使所有检查通过、即使只是改一行 CSS，也不能跳过确认
- **没有例外：`--files-only` 也是部署，同样需要确认**
