...
基于四大维度数值和模型状态，系统自动为每场比赛生成以下标签（可多选），用于前端高亮展示，帮助用户快速识别比赛特征。

| 标签 | 触发条件 | 示例文案 |
| :--- | :--- | :--- |
| 🔥 **绝对优势** | 综合实力 ≥ 25% | 主队绝对优势 |
| ⚠️ **模型打架** | 进球预测触发熔断（见第五部分） | 模型严重分歧，谨慎 |
| 💰 **过热风险** | 冷热指数 ≥ 1.40（见第四部分） | 市场过热，防冷 |
| 🧊 **冷门潜质** | 冷热指数 ≤ 0.85 | 逆向冷藏，关注 |
| 🎯 **实力均衡** | 综合实力在 -8% ~ +8% | 双方均衡，双选 |
| 🛡️ **防守大战** | 攻守实力 > 0.15 且 进球期望（M3.7 final）< 2.0 | 小球格局 |
| ⚡ **对攻大战** | 攻守实力 > 0.15 且 进球期望（M3.7 final）> 3.0 | 大球格局 |
| ✅ **高共识** | 胜平负离散度（lastWinDiscrete/lastDrawDiscrete/lastLoseDiscrete中最小值）< 7.0 | 机构分歧极小 |

**后端要求**：在返回每场比赛数据时，附加 `tags` 数组字段，如 `["绝对优势", "高共识"]`。

---

### 2.5 多场PK组合推荐引擎（增强功能）

当用户勾选2-4场比赛并点击“PK”时，系统自动计算以下三种推荐组合，并展示在底部面板。

#### 2.5.1 正路组合
- **选取逻辑**：筛选出 **综合实力 ≥ 15%** 且 **未触发“模型打架”** 且 **冷热指数 ≤ 1.2** 的场次。
- **推荐方向**：主胜（或根据综合实力正负推荐主/客胜）。
- **输出示例**：`利物浦(主胜) + 曼城(主胜)`

#### 2.5.2 博冷组合
- **选取逻辑**：筛选出 **冷热指数 ≤ 0.85** 或 **触发“模型打架”** 的场次。
- **推荐方向**：推荐冷门选项（如主队弱势则推荐客胜，反之亦然；模型打架场次推荐双选）。
- **输出示例**：`切尔西(客胜) + 横滨(平)`

#### 2.5.3 双选容错组合
- **选取逻辑**：筛选出 **实力均衡（-8% ~ +8%）** 的场次。
- **推荐方向**：胜/平 或 平/负 双选。
- **输出示例**：`鹿岛(胜/平) + 纽卡(负/平)`

**后端要求**：在PK响应中增加 `recommendations` 对象，包含三个组合的 `lineId` 列表及推荐方向文案。

---

### 2.6 动态合计面板增强

原有底部合计面板（绝对值之和）保留，同时新增以下统计项，提升多场横向对比的信息密度。

| 统计项 | 计算方式 | 业务价值 |
| :--- | :--- | :--- |
| **平均综合实力** | 勾选场次的综合实力算术平均值 | 判断整体选场正路程度 |
| **优势一致性** | （主队优势场次数）/（总场次数） | 判断是否全部偏向主队 |
| **模型健康度** | （触发熔断场次数）/（总场次数） | 风控提示 |
| **热度偏离度** | 冷热指数 > 1.2 或 < 0.8 的场次数 | 提示市场异常 |

**展示位置**：底部PK面板，位于“合计值”下方，以卡片或标签组形式呈现。

---

### 2.7 可视化图表方案（可选视图）

为提升数据可读性，前端可提供“图表视图”切换按钮，默认使用 ECharts 或 AntV 实现以下两种图形之一：

#### 方案A：横向分组柱状图
- **X轴**：勾选的比赛（最多5场）
- **Y轴**：四大维度数值（净胜球量化、胜平负交叉、综合实力、攻守实力），归一化至 [-1, 1]
- **图形**：每组包含4个柱子，正值绿色（越长越优），负值红色（越长越劣）
- **交互**：鼠标悬停显示具体数值
...
**建议**：移动端默认表格，平板/PC默认图表，并提供切换按钮。

---

### 2.8 数据接口扩展规范

为支撑上述增强功能，后端在现有PK接口响应中需增加以下字段：
  ...
  "matches": [
    {
      ...
      "guestTeam": "切尔西",
      "netGoal": 0.35,
      "resultCross": "胜45% 平30% 负25% (让0) + 让胜35% 让平30% 让负35% (让-1)",
      "compositeStrength": 0.22,
      "attackDefense": 0.15,
      "totalRank": 0.225,
      "tags": ["绝对优势", "高共识"],       // 新增
      "bigBallRatio": 65,                  // 用于进球PK，可选
      "modelConflict": false               // 是否触发熔断
    }
  ],
  "summary": {                             // 新增
    "sumNetGoal": 0.35,
    "sumResultCross": 0.18,
    "sumCompositeStrength": 0.22,
    "sumAttackDefense": 0.15,
    "avgCompositeStrength": 0.22,
    "homeAdvantageRatio": 1.0,
    "modelConflictCount": 0,
    "overheatCount": 0
  },
  "recommendations": {                     // 新增
    "positiveCombo": [
      { "lineId": "001", "direction": "主胜" }
    ],
    "coldCombo": [],
    "doubleChanceCombo": []
  }
