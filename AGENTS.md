...
## 🧠 新问题自学习协议（🔥 每次修复/返工/故障后立即执行，不等会话结束）

当 AI 执行了任何修复操作（fix commit、回退、补丁）时，**在同一轮回复中立即**：

1. **判断严重度并写入对应层级**：
   - P0（阻断核心功能/部署失败）→ **立即**追加到 `scripts/pre-commit-check.cjs` 新检查项 → ⚡ 下次提交就生效
   - P1（导致返工 ≥1 次）→ **立即**追加到对应 Skill 的 `references/lessons.md`
   - P2（偶发问题/新认知）→ **立即**调用 `update_memory` 记录

2. **更新问题追踪**：发现新的问题模式 → 追加到 `sucai/开发返工问题全面诊断报告.md`

3. **通知用户**：每轮回复结束时告知"本轮学到了什么 + 是否已写入"

> ⚠️ 禁止拖延到"会话结束前"——修复完成的那一刻就是最佳写入时机。
> 如果 AI 没有立即执行本协议，用户应主动提醒："skill 自动进化生效了么？"
...
修改 CSS/HTML/前端 JS → 本地启动 → Playwright 截图验证 → 确认无误 → 才能请求部署
