...
const MIDOU = 'https://midou310.com/mdsj';
const OUT = path.join(__dirname, 'data_all.json');
let token = null,
  lastLogin = 0;
...
async function login() {
  ...
  const res = await httpGet(MIDOU + '/gduser/login.do', { mobile: env.MIDOU_MOBILE, password: env.MIDOU_PASSWORD });
  if (res.code === 1) {
...
async function crawlAll() {
  ...
  if (!env.MIDOU_MOBILE || !env.MIDOU_PASSWORD) {
    console.error('缺少.env配置');
    process.exit(1);
  }
  ...
  const start = new Date('2026-03-19T00:00:00+08:00');
  const end = new Date();
  const allM = {},
    allR = {};
  let totalM = 0,
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    try {
      ...
      const mr = await httpGet(
        MIDOU + '/score/footballDataList.do',
      ...
      function getDayOfWeek(ds) {
        return new Date(ds.slice(0, 10).replace(/-/g, '/') + ' 00:00:00').getDay();
      }
      function fmtDate(dd) {
        return (
          dd.getFullYear() +
      ...
      matches.forEach(function (m) {
        ...
        if (!md || md.length !== 10) {
          ...
          if (st.length >= 5) {
            ...
            const stDate = new Date(
              nowY + '-' + st.slice(0, 2) + '-' + st.slice(3, 5) + 'T' + st.slice(6, 11) + ':00+08:00',
            );
        ...
        const wp = m.num ? m.num.slice(0, 2) : '';
        const eDay = weekMap[wp[1]]; // 第2个字符: 一/二/三...
        if (eDay !== undefined && md.length === 10) {
          const aDay = getDayOfWeek(md);
          if (aDay !== eDay) {
            // 寻找最近匹配日期
            const d = new Date(md + 'T00:00:00+08:00');
            let best = null,
              bestD = 99;
            for (let o = -6; o <= 6; o++) {
              const dd = new Date(d);
      ...
      const activeMatches = matches.filter(function (m) {
        return m.matchStatus !== 0 || m.recommNum > 0;
      });
      for (let i = 0; i < activeMatches.length; i++) {
        const m = activeMatches[i];
        const mid = String(m.matchId || m.dataId || '');
        try {
          const rr = await httpGet(
            MIDOU + '/score/getExpertRecommData.do',
