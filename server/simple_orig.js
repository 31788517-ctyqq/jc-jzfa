...
function normalizeRecs(recs) {
  return recs.map(function (x) {
    let r = x.rs !== undefined ? x.rs : x.result !== undefined ? x.result : null;
    if (r === 2) r = null;
...
app.post('/api', function (req, res) {
  ...
  if (a === 'plan-list') {
    ...
    const allM = [];
    const mKeys = Object.keys(data.m || {});
    for (let k = 0; k < mKeys.length; k++) {
      var m = data.m[mKeys[k]];
      if ((m.date || '').slice(0, 10) === dateStr) allM.push(m);
    }
    const recomm = data.recGroups || data.r || {};
    const plans = [];
    for (var i = 0; i < allM.length; i++) {
      ...
      for (let j = 1; j < recs.length; j++) {
        if (recs[j].num > top.num) top = recs[j];
      }
      plans.push({
        planId: 'p_' + m.matchId,
        homeName: m.homeName,
        ...
        startTime: m.startTime || '',
        matchStatus: m.matchStatus,
        mainDirection: top.type || '',
        expertCount: top.num || 0,
        playType: recs.length >= 2 ? '混合投注' : '单关投注',
        matchCount: 1,
        passType: '1串1',
        betCount: top.num || 1,
        multiplier: 1,
        ticketCount: 1,
        amount: (top.num || 1) * 2,
        maxPrize: Math.round((top.num || 1) * 2 * (1.8 + Math.random() * 2.2)),
  ...
  if (a === 'fix-data') {
    const result = fixMatchDates(data.m);
    // 回写到 data.json
    try {
      fs.writeFileSync(path.join(__dirname, 'data.json'), JSON.stringify({ m: data.m, r: data.r }));
      result.saved = true;
