...
function run(adp) {
  ...
  function scoreToResult(sc) {
    if (!sc) return '';
    const p = String(sc).replace(/-/g, ':').split(':');
  ...
  adp.transaction(function () {
    rows.forEach(function (r) {
      try {
        ...
        const matchNum = r.matchNum || '';
        const actualResult = scoreToResult(r.actual_score);
        const actualHG = r.actual_home_goals !== null ? r.actual_home_goals : 0;
        const actualAG = r.actual_away_goals !== null ? r.actual_away_goals : 0;
        const actSpf = r.actual_spf || '';

        const models = [];
        if (r.gs_top_score) {
          const gsDir = scoreToResult(r.gs_top_score);
          models.push({
            pid: makePid('gs', mid),
            name: '功守道',
            ver: 'v1.0',
            dh: gsDir === actSpf ? 1 : 0,
            sh: r.gs_top_score === r.actual_score ? 1 : 0,
          });
        }
        if (r.pk_direction) {
          models.push({
        ...
        models.forEach(function (m) {
          const existing = adp.execOne('SELECT id FROM prediction_outcomes WHERE prediction_id = ?', [m.pid]);
          if (existing) {
            skipped++;
            return;
          }
  ...
  // Verify
  const v = adp.execAll(
    'SELECT model_name, COUNT(*) as cnt FROM prediction_outcomes GROUP BY model_name ORDER BY cnt DESC',
  );
  console.log('=== prediction_outcomes ===');
  v.forEach(function (x) {
    console.log('  ' + x.model_name + ': ' + x.cnt);
  });
  process.exit(0);
}
wait(0);
