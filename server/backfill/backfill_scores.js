...
async function main() {
  ...
  Object.keys(data.m).forEach(function (k) {
    ...
    const dd = (m.date || '').slice(0, 10);
    if (!dd) return;
    // Only backfill matches that have recommendations with results (finished matches)
    const rawRecs = data.r['m_' + m.matchId] || data.r[m.matchId] || [];
    if (rawRecs.length === 0) return;
    const hasResult = rawRecs.some(function (r) {
  ...
  for (var dateKey in dates) {
    ...
    const timestamp = new Date(dateKey + 'T00:00:00+08:00').getTime();
    try {
      ...
      if (matchRes.code === 1 && matchRes.data) {
        ...
        matchRes.data.forEach(function (m2) {
          const id = String(m2.matchId || m2.dataId || '');
          apiMap[id] = m2;
        });
        ...
        needFill.forEach(function (item) {
          ...
          const apiMatch = apiMap[item.match.matchId];
          if (!apiMatch) {
            console.log('  Not found in API:', item.match.num);
            return;
          }

          const old = data.m[item.key];
          const score = apiMatch.score || old.score || '';
          const half = apiMatch.halfScore || old.halfScore || '';
          if (score || half) {
            data.m[item.key] = {
              matchId: old.matchId,
              num: old.num || '',
