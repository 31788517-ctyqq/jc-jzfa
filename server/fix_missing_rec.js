...
async function main() {
  ...
  if (login.code !== 1) {
    console.log('Login fail:', login.msg);
    process.exit(1);
  }
  ...
  Object.keys(d.m).forEach(function (k) {
    ...
    const mid = m.matchId;
    const rk = 'm_' + mid;
    if (m.recommNum > 0 && (!d.r[rk] || d.r[rk].length === 0))
      missing.push({ matchId: mid, num: m.num, recommNum: m.recommNum, homeName: m.homeName });
  });

  if (missing.length === 0) {
    console.log('No missing recommends');
    process.exit(0);
  }
  console.log('Missing recommends for', missing.length, 'matches:');
  missing.forEach(function (x) {
    console.log(' ', x.matchId, x.num, x.homeName, 'recommNum=' + x.recommNum);
  });
  ...
  for (let i = 0; i < missing.length; i++) {
    ...
    try {
      const rr = await get(
        ...
        { dataId: x.matchId, type: 0 },
        { Cookie: 'token=' + token },
      );
      if (rr.code === 1 && rr.data && rr.data.length) {
        const recs = (rr.data || [])
          .filter(function (r) {
            return r && r.type && r.num > 0;
          })
          .map(function (r) {
            return { type: r.type, num: r.num, result: r.result !== undefined ? r.result : null };
          });
