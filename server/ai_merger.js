...
function textSimilarity(a, b) {
  ...
  const setA = {},
    setB = {};
  for (let i = 0; i < sa.length - 1; i++) setA[sa.slice(i, i + 2)] = true;
  for (let j = 0; j < sb.length - 1; j++) setB[sb.slice(j, j + 2)] = true;
  let overlap = 0,
    total = 0;
  for (var k in setA) {
    total++;
  ...
  // 词级重合加权（中文用单字）
  const wordsA = sa.replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, '').split('');
  const wordsB = sb.replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, '').split('');
  let setWA = {},
    setWB = {},
    wordOverlap = 0;
  for (let wi = 0; wi < wordsA.length; wi++) setWA[wordsA[wi]] = true;
  for (let wj = 0; wj < wordsB.length; wj++) setWB[wordsB[wj]] = true;
  let wordTotal = Math.max(Object.keys(setWA).length, Object.keys(setWB).length);
  for (const wk in setWA) {
    if (setWB[wk]) wordOverlap++;
  }
  if (wordTotal === 0) wordTotal = 1;
  const wordSim = wordOverlap / wordTotal;

  const charSim = overlap / Math.max(total, 1);
...
function isOppositePrediction(dsDir, dbDir) {
  ...
  const ds = String(dsDir).trim(),
    db = String(dbDir).trim();
  // 胜平负相反
  if ((ds === '主胜' && (db === '客胜' || db === '主负')) || (db === '主胜' && (ds === '客胜' || ds === '主负')))
    return true;
  // 大小球相反
  if ((ds === '大球' && db === '小球') || (ds === '小球' && db === '大球')) return true;
  // 让球方向相反
  if ((ds === '让胜' && (db === '让负' || db === '让平')) || (db === '让胜' && (ds === '让负' || ds === '让平')))
    return true;
  return false;
}
...
function parseWinDrawLoss(text) {
  ...
  let m = str.match(/(\d+)\s*胜\s*(\d+)\s*平\s*(\d+)\s*负/);
...
function checkRecentFormConflict(dsStateSection, dbStateSection, homeTeam, awayTeam) {
  const dsHomeText = (dsStateSection || {})['主队近况'] || '';
  const dsAwayText = (dsStateSection || {})['客队近况'] || '';
  const dbHomeText = (dbStateSection || {})['主队近况'] || '';
  const dbAwayText = (dbStateSection || {})['客队近况'] || '';

  const dsHome = parseWinDrawLoss(dsHomeText);
  const dsAway = parseWinDrawLoss(dsAwayText);
  const dbHome = parseWinDrawLoss(dbHomeText);
  const dbAway = parseWinDrawLoss(dbAwayText);

  // 两个模型都必须有可解析的数据才做检测
  if (!dsHome && !dbHome && !dsAway && !dbAway) return null;

  const result = { detected: false, conflicts: {} };

  // 检测主队数据冲突
  if (dsHome && dbHome) {
    const homeConflict = !(dsHome.w === dbHome.w && dsHome.d === dbHome.d && dsHome.l === dbHome.l);
  ...
  // 检测客队数据冲突
  if (dsAway && dbAway) {
    const awayConflict = !(dsAway.w === dbAway.w && dsAway.d === dbAway.d && dsAway.l === dbAway.l);
...
function checkAttackDefenseConflict(dsRows, dbRows) {
  ...
  dsRows.forEach(function (row, i) {
    if (row && row.length >= 3) dsMap[row[0]] = row;
  });
  const dbMap = {};
  dbRows.forEach(function (row, i) {
    if (row && row.length >= 3) dbMap[row[0]] = row;
  });

  const conflictRows = [];
  let hasConflict = false;

  for (const label in dsMap) {
    const dsRow = dsMap[label],
      dbRow = dbMap[label];
    if (!dsRow) continue;
    const dsHome = String(dsRow[1] || ''),
      dsAway = String(dsRow[2] || '');
    const dbHome = dbRow ? String(dbRow[1] || '') : '';
    const dbAway = dbRow ? String(dbRow[2] || '') : '';

    // 解析数值
    const dsHomeNum = parseFloat(dsHome),
      dsAwayNum = parseFloat(dsAway);
    const dbHomeNum = parseFloat(dbHome),
      dbAwayNum = parseFloat(dbAway);
    ...
    const homeDiff = dbRow ? Math.abs(dsHomeNum - dbHomeNum) : 0;
    const awayDiff = dbRow ? Math.abs(dsAwayNum - dbAwayNum) : 0;
    const isConflict = homeDiff > 0.1 || awayDiff > 0.1;
  ...
  for (const lbl in dbMap) {
    if (!dsMap[lbl]) {
      const dbOnlyRow = dbMap[lbl];
      if (dbOnlyRow && dbOnlyRow.length >= 3) {
        conflictRows.push({
          label: lbl,
          dsHome: '',
          dsAway: '',
          dbHome: String(dbOnlyRow[1] || ''),
          dbAway: String(dbOnlyRow[2] || ''),
  ...
  // 按冲突行数决定 severity
  const conflictCount = conflictRows.filter(function (r) {
    return r.conflict;
  }).length;
  const severity = conflictCount === 0 ? 'none' : conflictCount <= 2 ? 'low' : 'medium';
...
function parsePredictions(predsArray) {
  ...
  predsArray.forEach(function (p) {
    const playType = (p['玩法'] || '').trim();
    const direction = (p['建议方向'] || '').trim();
...
function validateAndFixFromShuju(merged, matchInfo) {
  ...
  const shujuData = loadShujuForValidation(dateStr);
  if (!shujuData) return merged;

  const matchShuju = (shujuData.matches || {})[matchInfo.num || ''];
  if (!matchShuju || !matchShuju.recentForm) return merged;

  const rf = matchShuju.recentForm;
  ...
  if (merged['基础面'] && merged['基础面']['攻防全景数据']) {
    const oldRows = merged['基础面']['攻防全景数据'].rows || [];
    // 简单检测：如果现有 rows 长度不对或数值缺失，强制覆盖
    const needFix = oldRows.length < 4;
  ...
  // ⭐ 修正近期战绩主客队 WDL
  let formHomeStats, formAwayStats;
  const h6f = (rf.last6 || {}).home || {};
  const a6f = (rf.last6 || {}).away || {};
  if (h6f.wins !== undefined) {
    formHomeStats = h6f;
    formAwayStats = a6f;
  } else {
    formHomeStats = h10;
  ...
  const fixedHomeForm =
    '近6场' + (formHomeStats.wins || 0) + '胜' + (formHomeStats.draws || 0) + '平' + (formHomeStats.losses || 0) + '负';
  const fixedAwayForm =
    '近6场' + (formAwayStats.wins || 0) + '胜' + (formAwayStats.draws || 0) + '平' + (formAwayStats.losses || 0) + '负';

  if (merged['状态面']) {
    const oldHome = merged['状态面']['主队近况'] || '';
    const oldAway = merged['状态面']['客队近况'] || '';
    // 解析旧值中的 WDL
    const oldHomeWDL = parseWinDrawLoss(oldHome);
    const oldAwayWDL = parseWinDrawLoss(oldAway);
    const needFixHome =
      !oldHomeWDL ||
...
function mergeAnalyses(deepseekResult, doubaoResult, matchInfo) {
  const ds = deepseekResult.content || {};
  const db = doubaoResult.content || {};
  const dsConf = typeof ds.confidence === 'number' ? ds.confidence : 70;
  const dbConf = typeof db.confidence === 'number' ? db.confidence : 70;

  let merged = {};
  const conflicts = []; // 记录冲突信息

  // ── 步骤① 结构对齐 ──
  const sections = ['基础面', '状态面', '动机面', '对位面', '市场面', '核心看点'];
  sections.forEach(function (section) {
    const dsSection = ds[section] || {};
    const dbSection = db[section] || {};
    merged[section] = {};

    // 收集所有字段
    const fields = {};
    for (var k in dsSection) {
      if (dsSection.hasOwnProperty(k)) fields[k] = true;
    }
    for (var k in dbSection) {
      if (dbSection.hasOwnProperty(k)) fields[k] = true;
    }

    for (const field in fields) {
      const dsVal = dsSection[field];
      const dbVal = dbSection[field];
      ...
      if (typeof dsVal === 'string' && typeof dbVal === 'string') {
        const sim = textSimilarity(dsVal, dbVal);
        if (sim > 0.85) {
  ...
  // ── 步骤② 近期战绩交叉验证 ──
  const recentFormCheck = checkRecentFormConflict(
    ds['状态面'] || {},
    db['状态面'] || {},
  ...
  // ── 步骤②b 攻防全景数据交叉验证 ──
  const adDsRows = merged['基础面']['_attackDefenseDsRows'];
  const adDbRows = merged['基础面']['_attackDefenseDbRows'];
  if (adDsRows && adDbRows) {
    const adCheck = checkAttackDefenseConflict(adDsRows, adDbRows);
  ...
  // ── 步骤③ 预测建议冲突裁决 ──
  const dsPreds = parsePredictions(ds['预测建议'] || []);
  const dbPreds = parsePredictions(db['预测建议'] || []);
  merged['预测建议'] = [];

  const playTypes = {};
  for (var pt in dsPreds) {
    if (dsPreds.hasOwnProperty(pt)) playTypes[pt] = true;
  }
  for (var pt in dbPreds) {
    if (dbPreds.hasOwnProperty(pt)) playTypes[pt] = true;
  }

  let predictionConflict = false; // 是否发生方向冲突

  for (const playType in playTypes) {
    const dsDir = dsPreds[playType] ? dsPreds[playType].direction : '';
    const dbDir = dbPreds[playType] ? dbPreds[playType].direction : '';
    const dsLogic = dsPreds[playType] ? dsPreds[playType].logic : '';
    const dbLogic = dbPreds[playType] ? dbPreds[playType].logic : '';

    let finalDirection = dsDir || dbDir;
    let finalLogic = dsLogic || dbLogic;
    let conflictNote = '';

    if (dsDir && dbDir && isOppositePrediction(dsDir, dbDir)) {
      // 预测方向相反
      predictionConflict = true;
      const confGap = Math.abs(dsConf - dbConf);
      if (confGap > 30) {
        // 一方明显更有信心 → 取高信心模型
        ...
        conflictNote =
          '交叉验证：另一模型持不同观点（' +
          (dsConf >= dbConf ? '豆包倾向' + dbDir : 'DeepSeek倾向' + dsDir) +
          '），已采用高置信度模型建议';
      } else {
    ...
    // 如果有冲突提示，加入核心逻辑
    if (conflictNote) {
      const lastPred = merged['预测建议'][merged['预测建议'].length - 1];
  ...
  if (predictionConflict && merged['核心看点']) {
    const existingRemind = merged['核心看点']['变数提醒'] || '';
    ...
    if (conflictSummary && existingRemind.indexOf(conflictSummary) < 0) {
      merged['核心看点']['变数提醒'] = existingRemind ? existingRemind + ' ' + conflictSummary : conflictSummary;
    }
  }
  ...
  // ── 步骤⑤ JSON 重组 ──
  // 综合 confidence
  const finalConf = Math.round((dsConf + dbConf) / 2);
