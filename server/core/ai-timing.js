...
function updateTimingStats(deepseekMs, doubaoMs, mergeMs) {
  try {
    let timing = {
      ...
      doubao_avg_ms: 0,
      merge_avg_ms: 0,
      total_estimated_sec: 45,
      sample_count: 0,
      last_updated: '',
    };
    if (fs.existsSync(TIMING_PATH)) {
      try {
        timing = JSON.parse(fs.readFileSync(TIMING_PATH, 'utf8'));
      } catch (e) {}
    }
    const n = timing.sample_count || 0;
    const decay = n > 5 ? 0.9 : n > 0 ? 0.7 : 0;
    timing.deepseek_avg_ms =
      decay > 0 ? Math.round(timing.deepseek_avg_ms * decay + deepseekMs * (1 - decay)) : deepseekMs;
    ...
    timing.total_estimated_sec =
      Math.ceil((Math.max(timing.deepseek_avg_ms, timing.doubao_avg_ms) + timing.merge_avg_ms) / 1000) + 3;
