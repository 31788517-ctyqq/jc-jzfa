  ...
  return new Promise((resolve) => {
    try {
      ...
      _socket.on('error', (e) => {
        ...
        // 排空队列，用内存模式执行
        const queue = _commandQueue.splice(0);
        queue.forEach(function (item) { _execMem(item.cmd, item.resolve, item.reject); });
        resolve();
      });
      ...
      _socket.on('data', (data) => {
        _responseBuffer += data.toString('binary');
        _processBuffer();
      });
