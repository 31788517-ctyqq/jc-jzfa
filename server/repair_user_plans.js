...
// ── 处理所有方案文件 ──
var files = fs.readdirSync(PLANS_DIR).filter(function (f) {
...
files.forEach(function (fname) {
  ...
  try {
    ...
    plans = plans.map(function (p) {
      ...
      var hasSubs = (r.matches || []).every(function (m) {
    ...
    if (changed) {
      ...
      plans.forEach(function (p, i) {
        var subs = (p.matches || []).filter(function (m) {
          return m.subResults && m.subResults.length > 0;
        }).length;
        var won = (p.matches || []).filter(function (m) {
          return m.isMatchWon;
        }).length;
        console.log(
          '    [' +
      ...
      var hasSubs = plans.every(function (p) {
