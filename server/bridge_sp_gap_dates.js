...
for (const fname of files) {
  try {
    ...
    matchesByDate[date].push({
      ...
      homeName: data.home || '',
      visitName: data.away || '',
      leagueName: (data.matchNum || '')
        .replace(/周[一二三四五六日]\d{3}\s*/, '')
        .replace(/>/g, '')
        .trim(),
      date,
...
for (const date of TARGET_DATES) {
  ...
  for (const sp of matches) {
    ...
    if (existing) {
      ...
      if (!existing.homeName || existing.homeName === '') {
        existing.homeName = sp.homeName;
        changed = true;
      }
      if (!existing.visitName || existing.visitName === '') {
        existing.visitName = sp.visitName;
        changed = true;
      }
      if (existing.matchStatus < 2) {
        existing.matchStatus = 2;
        changed = true;
      }
      if (!existing.num) {
        existing.num = sp.num;
