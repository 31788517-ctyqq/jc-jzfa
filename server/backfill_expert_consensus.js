 ...
 */
var fs=require('fs'),path=require('path'),database=require('./database');
...
var rMap=data.r||{},mMap=data.m||{};
var keys=Object.keys(rMap).filter(function(k){return Array.isArray(rMap[k])&&rMap[k].length>0});
console.log('有推荐数据的比赛: '+keys.length);

// ★ 等待数据库初始化（兼容 sql.js 异步加载）
function waitForDb(cb) {
  database.initDatabase();
  if (database.isAvailable()) { cb(database.getAdapter()); return; }
  var start = Date.now();
  function check() {
    if (database.isAvailable()) { cb(database.getAdapter()); return; }
    if (Date.now() - start > 30000) { console.log('[db] 超时'); cb(null); return; }
...
if(!adp){console.error('DB不可用');process.exit(1)}

// 聚合推荐 → 专家共识
// ★ V9: 增加 RQSPF 类型匹配（让胜/让平/让负），midou310主力推荐类型
function computeConsensus(recs){
  var dirs={home:0,draw:0,away:0},total=0;
  recs.forEach(function(r){
    var type=r.type||'',num=Number(r.num)||1;
    total+=num;
  ...
  var top='home',topCount=dirs.home;
  ...
  return {
    ...
    homeCount:dirs.home,drawCount:dirs.draw,awayCount:dirs.away,total:total
  };
}
...
keys.forEach(function(k){
  try{
    ...
    if(dryRun){
      stats.new++;
      if(stats.new<=5) console.log(m.homeName+' vs '+m.visitName+' h:'+cons.homeCount+' d:'+cons.drawCount+' a:'+cons.awayCount+' -> '+cons.direction+'('+(cons.confidence*100).toFixed(0)+'%) '+cons.consensusTag);
      return;
    }
...
backfiller.backfill(adp,{dryRun:false}).then(function(r){
  ...
  var rh=adp.execAll("SELECT model_name,COUNT(*) as total,SUM(direction_hit) as hits FROM prediction_outcomes WHERE model_name='专家共识' GROUP BY model_name");
  rh.forEach(function(x){console.log('命中率: '+(x.total>0?(x.hits/x.total*100).toFixed(1):'0')+'% ('+x.hits+'/'+x.total+')')});
  console.log('\n══════════════════════════ 完成');
});
