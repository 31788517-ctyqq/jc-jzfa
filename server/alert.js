...
async function sendMail(subject, bodyHtml) {
  ...
  try {
    await tp.sendMail({
      ...
      html:
        '<div style="font-family:Microsoft YaHei,sans-serif;padding:16px;max-width:600px;">' +
        '<h3 style="color:#C20003;margin:0 0 12px;">' +
        subject +
        '</h3>' +
...
async function crawlFailed(errorMsg, context) {
  const html =
    ...
    '<td style="padding:8px 12px;background:#FFF5F5;">' +
    escapeHtml(errorMsg) +
    '</td></tr>' +
    (context
      ? '<tr><td style="padding:8px 12px;border-left:3px solid #eee;"><b>上下文</b></td>' +
...
async function loginFailed(errorMsg) {
  const html =
    '<p style="color:#F44336;">登录米斗数据失败，爬虫无法获取数据。</p>' +
    '<p><b>错误:</b> ' +
    escapeHtml(errorMsg) +
    '</p>' +
...
async function dataAnomaly(description, detail) {
  const html =
    '<p style="color:#FF9800;">数据文件出现异常，可能影响服务正常使用。</p>' +
    '<p><b>描述:</b> ' +
    escapeHtml(description) +
    '</p>' +
    (detail
      ? '<pre style="background:#f5f5f5;padding:12px;border-radius:4px;overflow:auto;">' + escapeHtml(detail) + '</pre>'
      : '');
...
async function aiFailed(errorMsg, matchInfo) {
  const html =
    '<p>AI 深度分析（DeepSeek + 豆包）执行失败。</p>' +
    '<p><b>比赛:</b> ' +
    escapeHtml(matchInfo || '未知') +
    '</p>' +
    '<p><b>错误:</b> ' +
    escapeHtml(errorMsg) +
    '</p>';
...
async function test() {
  ...
  try {
    ...
    await sendMail(
      '✅ 告警系统已就绪',
      '<p>JC-ZJFA 竞彩监控系统的邮件告警功能已成功配置并启用。</p><p>您将从现在起收到系统异常告警邮件。</p>',
    );
    return true;
  } catch (e) {
