...
const https = require('https'),
  fs = require('fs'),
  path = require('path');
const logger = require('./logger').child('refetch_rec');
...
async function main() {
  ...
  // 登录
  log('Logging in...');
  let login = await get('https://midou310.com/mdsj/gduser/login.do', {
    mobile: env.MIDOU_MOBILE,
    password: env.MIDOU_PASSWORD,
  });
  if (login.code !== 1) {
    log('Login FAIL: ' + JSON.stringify(login));
    process.exit(1);
  }
  ...
  for (let i = 0; i < allMatches.length; i++) {
    ...
    const pct = Math.round((i / allMatches.length) * 100);
    if (i % 50 === 0)
      log(
        'Progress: ' +
          i +
    ...
    try {
      ...
      if (rr.code !== 1) {
        log('API fail ' + m.matchId + ' code=' + rr.code);
        failures++;
        await sleep(300);
        continue;
      }
      ...
      if (e.message.indexOf('token') >= 0 || e.message.indexOf('login') >= 0) {
        try {
          login = await get('https://midou310.com/mdsj/gduser/login.do', {
            mobile: env.MIDOU_MOBILE,
            password: env.MIDOU_PASSWORD,
          });
    ...
    if (i > 0 && i % 200 === 0) {
      try {
        ...
        log('Checkpoint save FAIL: ' + e.message);
      }
      // 重新登录保持session
      try {
        login = await get('https://midou310.com/mdsj/gduser/login.do', {
        ...
        log('Refresh login fail, continue with existing token');
      }
    }
  }
