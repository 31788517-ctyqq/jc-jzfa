...
describe('odds-movement — analyzeMovement 盘口位移检测', () => {
  it('无效赔率 (≤1.0) → 返回默认结果 (盘口稳定)', () => {
    const result = analyzeMovement({ home: 1.0, draw: 3.0, away: 4.0 }, { home: 1.0, draw: 3.0, away: 4.0 }, 0);
  ...
  it('盘口稳定 (probShift < 0.02) → direction=盘口稳定', () => {
    const result = analyzeMovement({ home: 2.1, draw: 3.2, away: 3.5 }, { home: 2.08, draw: 3.25, away: 3.45 }, 0.1);
    expect(result.direction).toBe('盘口稳定');
    expect(result.severity).toBe('none');
    expect(result.penalty).toBe(0);
  });
  ...
  it('主胜微降水 + 模型看客胜 (pwScore < -0.08) → penalty=8', () => {
    // 小幅变化但方向与模型相反
    const result = analyzeMovement(
      { home: 2.3, draw: 3.2, away: 3.1 },
      { home: 2.15, draw: 3.3, away: 3.25 },
      -0.15, // 模型看客胜，矛盾
    );
    expect(result.direction).toBe('主胜微降水');
  ...
  it('主胜降水 (probShift > 0.05, pwScore≥0) → direction标记市场看好', () => {
    const result = analyzeMovement({ home: 3.0, draw: 3.2, away: 2.3 }, { home: 1.8, draw: 3.5, away: 4.5 }, 0.1);
    expect(result.direction).toBe('主胜降水（市场看好主队）');
    expect(result.penalty).toBe(0);
  });

  it('主胜降水 + 模型看客胜 (pwScore < -0.1) → penalty=15, severity=significant', () => {
    const result = analyzeMovement({ home: 3.0, draw: 3.2, away: 2.3 }, { home: 1.8, draw: 3.5, away: 4.5 }, -0.2);
    expect(result.direction).toBe('主胜降水（市场看好主队）');
  ...
  it('主胜升水 (probShift < -0.05, pwScore≤0) → 市场看衰主队', () => {
    const result = analyzeMovement({ home: 2.0, draw: 3.2, away: 3.8 }, { home: 3.0, draw: 3.2, away: 2.3 }, -0.1);
    expect(result.direction).toBe('主胜升水（市场看衰主队）');
    expect(result.penalty).toBe(0);
  });

  it('主胜升水 + 模型看主胜 (pwScore > 0.1) → penalty=15', () => {
    const result = analyzeMovement({ home: 2.0, draw: 3.2, away: 3.8 }, { home: 3.0, draw: 3.2, away: 2.3 }, 0.2);
    expect(result.direction).toBe('主胜升水（市场看衰主队）');
    expect(result.penalty).toBe(15);
    expect(result.severity).toBe('significant');
  });
  ...
  it('主胜微升水 (probShift < -0.02, pwScore≤0) → 无penalty', () => {
    ...
    const result = analyzeMovement({ home: 2.3, draw: 3.2, away: 3.0 }, { home: 2.5, draw: 3.2, away: 2.8 }, -0.05);
    expect(result.direction).toBe('主胜微升水');
    expect(result.penalty).toBe(0);
  });

  it('主胜微升水 + pwScore > 0.08 → penalty=8', () => {
    const result = analyzeMovement({ home: 2.2, draw: 3.2, away: 3.2 }, { home: 2.5, draw: 3.2, away: 2.8 }, 0.12);
    expect(result.direction).toBe('主胜微升水');
    expect(result.penalty).toBe(8);
  ...
  it('返回隐含概率和位移值', () => {
    const result = analyzeMovement({ home: 2.0, draw: 3.2, away: 4.0 }, { home: 1.8, draw: 3.5, away: 5.0 }, 0.1);
  ...
  it('大幅变化 → severity=significant', () => {
    const result = analyzeMovement({ home: 3.5, draw: 3.2, away: 2.1 }, { home: 1.6, draw: 4.0, away: 5.5 }, 0);
    expect(result.severity).toBe('significant');
...
describe('odds-movement — checkEuroAsiaConsistency 欧亚一致性', () => {
  ...
  it('平手盘 (rq=0) → 亚盘无方向，欧赔方向决定', () => {
    const result = checkEuroAsiaConsistency({ home: 2.5, draw: 3.2, away: 2.8 }, 0, 0);
  ...
  it('极端赔率边界 → 不崩溃', () => {
    const result = checkEuroAsiaConsistency({ home: 15.0, draw: 8.0, away: 1.15 }, 2.5, -0.5);
