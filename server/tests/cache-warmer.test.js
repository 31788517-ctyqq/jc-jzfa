...
describe('cache-warmer — warmUp 基础流程', () => {
  ...
  (isFast ? it.skip : it)('warmUp 尝试 6 步预热（可能有文件不存在的错误）', async () => {
    const stats = await warmUp();
    expect(stats.steps + stats.errors).toBe(6);
  }, 10000);

  (isFast ? it.skip : it)('自定义 log 函数被调用', async () => {
    const calls = [];
    await warmUp({
      log: function (msg) {
        calls.push(msg);
  ...
  (isFast ? it.skip : it)('files 对象包含 6 个缓存文件', async () => {
    const stats = await warmUp();
  ...
  (isFast ? it.skip : it)('每个文件结果有 ok 状态', async () => {
    const stats = await warmUp();
  ...
  (isFast ? it.skip : it)('连续调用不抛异常', async () => {
    await expect(warmUp()).resolves.toBeDefined();
    await expect(warmUp()).resolves.toBeDefined();
  }, 20000);

  (isFast ? it.skip : it)('无参数调用使用默认 log', async () => {
    await expect(warmUp()).resolves.toBeDefined();
  }, 10000);
});
