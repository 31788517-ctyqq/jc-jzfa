  ...
  if (oddsEntry) {
    ...
    if (bqcSource) {
      ...
      if (Array.isArray(bqcSource)) {
        bqcSource.forEach(function (b) { bqcMap[b.combo || b.label || b.key] = b.odds; });
...
const THURSDAY203_ALLPLAYS = {
  ...
  totalGoals: { '0': 45, '1': 11.5, '2': 6.3, '3': 4, '4': 4,
    '5': 5.15, '6': 7.5, '7+': 6.75 },
  halfFull: {
...
const THURSDAY201_ALLPLAYS = {
  ...
  rqspf: { home: 4.50, draw: 3.80, away: 1.60, handicap: -1 },
  scores: { '1:0': 7.0, '2:0': 8.0, '2:1': 8.5 },
  totalGoals: { '0': 13, '1': 5.25, '2': 3.50, '3': 3.00 },
  halfFull: { hh: 2.80, hd: 5.50, ha: 15.0, dh: 4.20, dd: 4.80, da: 5.00, ah: 25.0, ad: 12.0, aa: 7.00 },
};
...
describe('match-odds handler — 响应结构', () => {
  ...
  describe('周四203 边界场景: SPF=null + RQSPF + 全玩法', () => {
    ...
    it('RQSPF 返回 2.21/4.20/2.28', () => {
      ...
      expect(resp.rqspfList[0].draw).toBe(4.20);
  ...
  describe('常规场次: SPF + RQSPF + 全玩法', () => {
    ...
    it('SPF 返回有效赔率', () => {
      expect(resp.spf.home).toBe(2.10);
      expect(resp.spf.draw).toBe(3.30);
      expect(resp.spf.away).toBe(3.20);
    });
  ...
  describe('key 命名兼容: scores → bf, totalGoals → jqs, halfFull → bqc', () => {
    ...
    it('halfFull 对象正确映射为 bqc 数组', () => {
      const entry = { halfFull: { hh: 2.80, dd: 4.80, aa: 7.00 } };
      const allplays = makeAllplaysDay('2026-06-01', { '001': entry });
      const resp = buildMatchOddsResponse('001', '2026-06-01', allplays);
      expect(resp.bqc.length).toBe(3);
      expect(resp.bqc[0]).toEqual({ combo: '胜胜', odds: 2.80 });
    });
...
describe('batch-match-odds handler — 响应结构', () => {
  ...
  it('周四201 spf 和 rqspf 均有效', () => {
    const resp = buildBatchMatchOddsResponse('2040091',
  ...
  it('无 odds 数据的场次返回 null 值', () => {
    const resp = buildBatchMatchOddsResponse('2040999',
