...
function waitForDb() {
  return new Promise(resolve => {
    ...
    if (database.isAvailable()) { resolve(database.getAdapter()); return; }
    const start = Date.now();
    function check() {
      if (database.isAvailable()) { resolve(database.getAdapter()); return; }
      if (Date.now() - start > 30000) { console.error('数据库初始化超时'); process.exit(1); }
...
(async function() {
  ...
  let uniTotal = 0, uniSkipped = 0, uniErrors = 0;
  const modelCounts = {};
          ...
          try {
            adp.execRun(
              `INSERT INTO unified_predictions
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              row.matchNum || '', date, mid, '功守道', 'v1.0', predId,
              gsDir, row.gs_top_percent || 50, row.gs_top_score || null,
              row.gs_scores_json || null, row.pk_fusion_consensus || null, row.created_at || date
            );
            uniTotal++;
          ...
          try {
            adp.execRun(
              `INSERT INTO unified_predictions
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              row.matchNum || '', date, mid, 'PK评分', 'v1.0', predId,
              pkDir, row.pk_composite_score || 50,
              mapOverUnder(row.pk_goal_direction), null,
              JSON.stringify({
                composite: row.pk_composite_score, power: row.pk_power_score,
                goal: row.pk_goal_score, heat: row.pk_heat_score,
                stability: row.pk_stability_score, value: row.pk_value_score,
                ev_home: row.pk_ev_home, ev_draw: row.pk_ev_draw, ev_away: row.pk_ev_away,
              }),
              row.pk_fusion_consensus || null, row.created_at || date
            );
            uniTotal++;
...
function mapDirection(cn) {
  ...
  if (cn === '客胜' || cn.startsWith('客胜') || cn === '客队不败' || cn === '客队胜' || cn === '平负' || cn.includes('客胜')) return 'away';
