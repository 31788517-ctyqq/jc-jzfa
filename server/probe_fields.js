...
let envTxt = '';
try {
  envTxt = fs.readFileSync(path.join(__dirname, '.env'), 'utf-8');
} catch (e) {}
envTxt.split('\n').forEach(function (l) {
  const p = l.trim().split('=');
  if (p.length === 2) process.env[p[0]] = p[1];
});

const MOBILE = process.env.MIDOU_MOBILE,
  PASSWORD = process.env.MIDOU_PASSWORD;
if (!MOBILE || !PASSWORD) {
  console.error('Need MIDOU_MOBILE/MIDOU_PASSWORD');
  process.exit(1);
}

async function main() {
  const r = await get('https://midou310.com/mdsj/gduser/login.do', { mobile: MOBILE, password: PASSWORD });
  ...
  matches.forEach(function (m) {
    ...
    if (m.score && m.score.indexOf(':') > 0 && m.score.indexOf('-') < 0) {
      console.log(
        ...
        Object.keys(m)
          .filter(function (k) {
            return k.toLowerCase().indexOf('score') >= 0 || k.toLowerCase().indexOf('half') >= 0;
          })
          .join(','),
      );
    }
