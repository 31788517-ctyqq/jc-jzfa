...
const oddsCache = {},
  oddsCacheTime = {},
  oddsCacheTTL = 1800000;
// 按日期缓存500.com全量赔率
let odds500Cache = {},
  odds500Date = '';
// 已从odds_history加载的日期（防止异步抓取覆盖历史数据）
const oddsHistoryLoaded = {};
// 方案快照冻结缓存
const planSnapshots = {};
app.use(compression());
app.use(cors());
...
function fixMatchDates(allM) {
  ...
  Object.keys(allM).forEach(function (k) {
    ...
    const wp = m.num.slice(0, 2);
    const eDay = WEEK_NAMES[wp];
    if (eDay === undefined) return;
    const aDay = getDayOfWeek(m.date);
    if (aDay === eDay) return;
    if (aDay < 0) return;
    const d = new Date(m.date.slice(0, 10) + 'T00:00:00+08:00');
    let best = null,
      bestD = 99;
    for (let o = -6; o <= 6; o++) {
      const dd = new Date(d);
      dd.setDate(dd.getDate() + o);
      if (dd.getDay() === eDay) {
        const dist = o < 0 ? -o : o;
        if (dist < bestD) {
          bestD = dist;
...
try {
  ...
  // 启动时自动修复日期不匹配
  const fixResult = fixMatchDates(data.m);
  ...
  Object.keys(data.m).forEach(function (k) {
    ...
    const recs = normalizeRecs(data.r['m_' + m.matchId] || data.r[m.matchId] || []);
    if (
  ...
  console.log('No data.json:', e.message);
}
...
let liveDate = '';
try {
  const live = JSON.parse(fs.readFileSync(path.join(__dirname, 'live_scores.json'), 'utf8'));
  liveDate = live.date || '';
  (live.matches || []).forEach(function (m) {
    liveScores[m.matchId] = {
      ...
      visitScore: m.visitScore,
      recommNum: m.recommNum,
      homeName: m.homeName,
  ...
  console.log('No live_scores.json');
}

function mergeLiveScore(m) {
  const s = liveScores[m.matchId];
  if (!s) return m;
  const merged = Object.assign({}, m, s);
  merged.dateLive = liveDate || s.date || '';
  return merged;
}

app.get('/health', function (req, res) {
  res.json({ status: 'ok', matches: Object.keys(data.m).length });
});
...
// 缓存
const cache = { rank: { data: null, time: 0, cat: '' } };
...
function getWeekDates() {
  ...
  Object.values(data.m).forEach(function (m) {
    if (!m) return;
    const w = m.num ? m.num.slice(0, 2) : '';
    if (!w || w.length < 2) return;
    const matchDate = (m.date || '').slice(5) || '';
    if (!matchDate) return;
    const key = w + '|' + matchDate;
    if (!seen[key]) {
      seen[key] = true;
      list.push({ weekNum: w, matchDate: matchDate, label: matchDate.replace('-', '/') + ' ' + w });
    }
  });
...
app.post('/api', function (req, res) {
  ...
  if (a === 'plan-list') {
    ...
    function extractSubOdds(oddsObj, direction) {
      ...
      if (direction.indexOf('总进球-') === 0) {
        ...
        for (let ni = 0; ni < nums.length; ni++) {
          const n = nums[ni].replace(/球/g, '').trim();
      ...
      if (direction.indexOf('、') >= 0 || direction.indexOf(',') >= 0) {
        ...
        for (let pi = 0; pi < parts.length; pi++) {
          const pd = parts[pi].trim();
          if (pd === '平') {
            if (oddsObj.spf) vals.push(oddsObj.spf.draw);
    ...
    function buildMatchObj(m, direction) {
      ...
      const matchedRecs = [];
      let subResults = []; // [{direction, result}] 用于前端着色
    ...
    function generatePlans() {
      ...
      if (m1a && m1b) {
        ...
        const mp1 = eff1 && eff2 ? Math.round(1000 * eff1 * eff2) : Math.round(1000 * 2.5);
        plans.push({
          planId: 'plan_' + dateStr + '_1',
          planName: '方案一',
      ...
      if (m2a && m2b) {
        ...
        const mp2 = eff1 && eff2 ? Math.round(1000 * eff1 * eff2) : Math.round(1000 * 2.5);
        plans.push({
          planId: 'plan_' + dateStr + '_2',
          planName: '方案二',
  ...
  if (a === 'income-stats') {
    const planFilter = d.plan || 'all';
    const daysFilter = parseInt(d.days) || 0; // 0=all
    const AMOUNT = 1000; // 方案金额固定1000元
    ...
    for (var d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      if (fs.existsSync(oddsFile)) {
        try {
          var raw = JSON.parse(fs.readFileSync(oddsFile, 'utf8'));
          histOdds = raw.odds || {};
        } catch (e) {}
      }
      ...
      function buildMatch(match, direction) {
        ...
        let fullMatch = null;
        for (let ri = 0; ri < recs.length; ri++) {
          if (recs[ri].type === direction) fullMatch = recs[ri];
        }
        if (fullMatch) {
          ...
          subDirs.forEach(function (sd) {
            ...
            for (var ri = 0; ri < recs.length; ri++) {
              if (recs[ri].type === s) found = recs[ri];
            }
            if (!found && s.indexOf('球') >= 0) {
              const num = s.replace(/球/g, '');
              for (var ri = 0; ri < recs.length; ri++) {
                if (recs[ri].type === '总进球-' + num) found = recs[ri];
              }
            }
            if (found) matchedRecs.push(found);
      ...
      dayPlans.forEach(function (pp) {
        ...
        let hasAllOdds = true;
        for (var mi = 0; mi < pp.matches.length; mi++) {
          const subOdds = extractIndividualOdds(pp.matches[mi].oddsObj, pp.matches[mi].direction);
          if (subOdds.length === 0) {
            hasAllOdds = false;
            continue;
          }
          ...
          if (N === 1) {
            ...
            // 双选: eff = sum / (2N) = sum / 4
            const sum = subOdds.reduce(function (a, b) {
              return a + b;
            }, 0);
        ...
        if (allWon) {
          prize =
            hasAllOdds && effectiveOdds.length === 2
              ? Math.round(AMOUNT * effectiveOdds[0] * effectiveOdds[1])
              : Math.round(AMOUNT * 3);
  ...
  if (a === 'match-list') {
      ...
      Object.keys(liveScores).forEach(function (k) {
        ...
        if (!seen[k])
          all.push({
            ...
            num: '',
            startTime: '',
            date: ls.date || liveDate || '',
          });
      });
    }
    // 合并实时比分
    all = all.map(function (m) {
      return mergeLiveScore(m);
    });
    // 补充推荐数
    all = all.map(function (m) {
      const recs = findRecommends(m.matchId);
      return Object.assign({}, m, {
        recommNum:
          m.recommNum ||
          recs.reduce(function (s, x) {
            return s + (x.num || 0);
          }, 0),
      });
    });
    // 补充500.com单关标识
    let needFetch500 = false;
    all = all.map(function (m) {
      const num = m.num || '';
      const fiveData = odds500Cache[num];
      let isSingle = false;
    ...
    all.sort(function (a, b) {
      ...
      const oa = order[a.matchStatus] !== undefined ? order[a.matchStatus] : 99;
      const ob = order[b.matchStatus] !== undefined ? order[b.matchStatus] : 99;
  ...
  if (a === 'recommend-trend') {
    ...
    let series = [],
      timeLabels = [];
    try {
      const trendFile = path.join(__dirname, 'trends.json');
      if (fs.existsSync(trendFile)) {
        const trends = JSON.parse(fs.readFileSync(trendFile, 'utf8'));
        const key = 'm_' + d.matchId;
        ...
        if (snaps.length > 0) {
          ...
          // 按当前总数排序取top5
          const top5 = Object.keys(typeMap)
            .sort(function (a, b) {
              return (typeMap[b] ? typeMap[b].num : 0) - (typeMap[a] ? typeMap[a].num : 0);
            })
            .slice(0, 5);
          series = series.filter(function (s) {
            return top5.indexOf(s.name) >= 0;
          });
        }
      }
    } catch (e) {}
  ...
  if (a === 'ranking-list') {
    const filterCat = d.category || null,
      filterDir = d.direction || null,
      reqDate = d.date || null;
    // 30s 缓存(按完整参数)
    const ck = filterCat + '|' + filterDir + '|' + (reqDate || '');
    ...
    if (reqDate) {
      ...
      for (let i = 0; i < 30; i++) {
        var ds = fmtDate(now);
        if (dateRecNum[ds] && dateRecNum[ds] > 0) {
          effectiveDate = ds;
          break;
        }
    ...
    Object.keys(data.m).forEach(function (k) {
      const m = data.m[k];
      if (!m) return;
      if (m.date === effectiveDate && !effectiveWeek) {
        effectiveWeek = m.num ? m.num.slice(0, 2) : '';
      }
    });
    const matchDateLabel = effectiveDate.slice(5).replace('-', '/') + ' ' + effectiveWeek;
    function inRange(dt) {
      return dt === effectiveDate;
    }
    // 分类函数
    ...
    Object.keys(categories).forEach(function (c) {
      categories[c].directions.sort(function (a, b) {
        return b.totalExpertCount - a.totalExpertCount;
      });
    });
    const CAT_ORDER = ['胜平负', '让球', '进球数', '半全场', '双选'];
    const sortedCats = {};
    CAT_ORDER.forEach(function (k) {
      if (categories[k]) sortedCats[k] = categories[k];
    });
    // 构建排名
    let list = [];
    if (filterDir && dirStats[filterDir]) {
      dirStats[filterDir].matches.forEach(function (x) {
        if (inRange(x.date)) list.push(x);
      });
    } else if (filterCat && categories[filterCat]) {
      const catDirs = {};
      categories[filterCat].directions.forEach(function (d) {
        catDirs[d.name] = true;
      });
      Object.keys(data.m).forEach(function (k) {
        const m = data.m[k];
        ...
        const recs = findRecommends(m.matchId).filter(function (r) {
          return catDirs[r.type] && r.num > 0;
        });
        if (recs.length) {
          const max = recs.reduce(function (a, b) {
            return b.num > a.num ? b : a;
          });
          list.push({
            matchId: m.matchId,
            homeName: m.homeName,
    ...
    const ranking = list.map(function (x, i) {
      x.rank = i + 1;
      return x;
    });
    const topExpertCount = list.length > 0 ? list[0].expertCount : 0;
    const respData = {
      code: 1,
      data: {
  ...
  if (a === 'hit-rate-stats') {
    ...
    Object.keys(recMap).forEach(function (t) {
      const v = recMap[t];
      ds.push({
        direction: t,
        totalRecommends: v.total,
        hitCount: v.hit,
        missCount: v.miss,
        hitRate: v.total > 0 ? Math.round((v.hit / v.total) * 1000) / 10 : 0,
      });
    });
    ds.sort(function (a, b) {
      return b.hitCount - a.hitCount;
    });
    const top3HitRate = fieldTopTotal > 0 ? Math.round((fieldTopHits / fieldTopTotal) * 1000) / 10 : 0;
    return res.json({
      code: 1,
      data: { directionStats: ds, top3HitRate: top3HitRate, totalMatchFields: fieldTopTotal, top3Hits: fieldTopHits },
    });
  }
  if (a === 'filter-stats') {
    var ls = new Set(),
      ...
      ms = new Set();
    Object.keys(data.r).forEach(function (k) {
      const m = data.m[k] || data.m[k.replace('m_', '')];
      if (!m) return;
      const r = normalizeRecs(data.r[k]).filter(function (x) {
        return x.result !== null;
      });
      if (r.length) {
        ms.add(k.replace('m_', ''));
        if (m.leagueName) ls.add(m.leagueName);
        r.forEach(function (x) {
          ds.add(x.type);
        });
      }
    });
    const lg = Array.from(ls).sort();
    let sl = 0;
    Object.keys(data.r).forEach(function (k) {
      const mId = k.replace('m_', '');
      const m = data.m[k] || data.m[mId];
      if (!m || m.matchStatus < 2) return;
      sl += normalizeRecs(data.r[k]).filter(function (x) {
        return x.result === null;
      }).length;
    });
    return res.json({
      code: 1,
      data: { matchCount: ms.size, leagueCount: ls.size, directionCount: ds.size, leagues: lg, staleCount: sl },
    });
  }
  if (a === 'filter-leagues') {
    var ls = new Set();
    Object.keys(data.m).forEach(function (k) {
      const m = data.m[k];
      if (m && m.leagueName) ls.add(m.leagueName);
    });
  ...
  if (a === 'hit-rate-filter') {
    var league = d.league || '',
      ...
      dt = d.directionType || '',
      dir = d.direction || '',
      rt = parseInt(d.rankTop) || 0,
      rankType = d.rankType || '全部',
      cutoff = null;
    if (tr !== 'all') {
      const cd = new Date(Date.now() - parseInt(tr) * 86400000);
      cutoff =
        cd.getFullYear() +
        '-' +
    ...
    function ct(t) {
      ...
      if (t == '胜' || t == '平' || t == '负') return '胜平负';
      if (t[0] == '让') return '让球';
      if (t == '胜胜' || t == '负负' || t.slice(0, 4) == '半全场-') return '半全场';
      if (t.slice(0, 4) == '总进球-') return '进球数';
      if (/^[\d,\u3001]+$/.test(t)) return '进球数';
      if (/[平胜负让球]/.test(t) && (t.indexOf(',') >= 0 || t.indexOf('\u3001') >= 0)) return '双选';
      return 'other';
    }
    function gd(dt) {
      const m = {};
    ...
    Object.keys(data.m).forEach(function (rawKey) {
      ...
      if (!dd) return;
      const recs = findRecommends(mid).filter(function (x) {
        return x.result !== null;
      });
      if (!recs.length) return;
      let mx = 0;
      recs.forEach(function (x) {
        if (x.num > mx) mx = x.num;
      });
    ...
    Object.keys(data.r).forEach(function (k) {
      ...
      // 记录全局最大值（不限方向）
      const dd = match.date ? match.date.slice(0, 10) : '';
      if (dd && r[0].num > (globalMaxDay[dd] || 0)) globalMaxDay[dd] = r[0].num;
      let keep = [];
      if (td) {
        // 先按方向过滤，再取该方向内的最大值
        const fr = r.filter(function (x) {
          return td.indexOf(x.type) >= 0 || td.indexOf(ct(x.type)) >= 0;
        });
        if (!fr.length) return;
        fr.sort(function (a, b) {
          return b.num - a.num;
        });
        const localMax = fr[0].num;
        fr.forEach(function (x) {
          if (x.num === localMax) keep.push(x);
        });
      } else {
    ...
    const dailyResults = [];
    const isDaily = rankType === '每天' && rt > 0;
    const isPerMatch = rankType === '每场' && rt > 0;
    let totalTc = 0,
      totalHc = 0;
    sortedDates.forEach(function (k) {
      ...
      if (isDaily) {
        ...
        const ranked = Object.keys(m.matchMax).sort(function (a, b) {
          return m.matchMax[b] - m.matchMax[a];
        });
    ...
    const hc = totalHc,
      tc = totalTc,
      hr = tc > 0 ? Math.round((totalHc / totalTc) * 1000) / 10 : 0;
    const p = [];
    if (league) p.push(league);
    if (tr == '30') p.push('近30天');
    else if (tr == '60') p.push('近60天');
    else if (tr == '90') p.push('近90天');
    if (dt === '综合排名') p.push('综合排名');
    if (dir && dt) p.push(dir);
    else if (dt && dt !== '综合排名') p.push(dt);
    if (rankType !== '全部' && rt > 0) {
      const rl = ['', '第一名', '前二名', '前三名', '前四名', '前五名', '前六名'];
    ...
    return res.json({
      ...
      data: {
        ...
        totalCount: tc,
        hitRate: hr,
        conditionSummary: p.length ? p.join(' | ') : '全部条件',
        detailList: detail,
  ...
  let aiCache = {};
  try {
    aiCache = JSON.parse(fs.readFileSync(path.join(__dirname, 'ai_cache.json'), 'utf8'));
  } catch (e) {}
  ...
  if (a === 'ai-predict') {
    ...
    // 如果有缓存直接返回
    if (aiCache[mid] && aiCache[mid].content)
      return res.json({
        code: 1,
        data: {
          matchId: mid,
          content: aiCache[mid].content,
          confidence: aiCache[mid].confidence || 0,
          fromCache: true,
        },
      });
    // 无缓存时异步触发生成（不阻塞响应）
    try {
      var ds = require('./deepseek');
      var m = data.m['m_' + mid] || data.m[mid];
      if (m) {
        ...
        ds.generateAnalysis({
          ...
          .then(function (r) {
            if (r.content) {
              aiCache[mid] = {
                content: r.content,
                confidence: (r.content && r.content.confidence) || 0,
                updatedAt: new Date().toISOString(),
              };
              fs.writeFileSync(path.join(__dirname, 'ai_cache.json'), JSON.stringify(aiCache));
              console.log('[ai] 缓存已写入', mid);
            }
          })
          .catch(function (e) {
            console.error('[ai] 生成失败', mid, e.message);
          });
        return;
      }
    } catch (e) {}
  ...
  if (a === 'ai-predict-status') {
    ...
    Object.keys(data.m).forEach(function (k) {
      const m = data.m[k];
      if (m && m.date && m.date.slice(0, 10) === td) {
        tm++;
        if (m.matchStatus >= 3) fm++;
      }
    });
    return res.json({
      code: 1,
      data: {
        ...
        totalMatches: tm,
        finishedMatches: fm,
        unfinishedMatches: tm - fm,
        canShowCards: tm - fm > 0,
      },
    });
  }
...
function loadOddsHistory() {
  const dir = path.join(__dirname, 'odds_history');
  if (!fs.existsSync(dir)) {
    console.log('No odds_history directory');
    return;
  }
  const files = fs
    .readdirSync(dir)
    .filter(function (f) {
      return f.endsWith('.json') && f !== 'batch_report.json';
    })
    .sort();
  let loaded = 0;
  const loadedKeys = {}; // 防止跨文件重复覆盖
  files.forEach(function (f) {
    try {
      const raw = fs.readFileSync(path.join(dir, f), 'utf8');
      const record = JSON.parse(raw);
      const dateStr = record.date;
      const oddsData = record.odds || {};
