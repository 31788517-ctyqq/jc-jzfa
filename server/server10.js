...
async function getRecommends(matchId) {
  ...
  if (res.code !== 1) throw new Error('fetch failed');
  return (res.data || [])
    .filter((i) => i && i.type && i.num > 0)
    .map((i) => ({ type: i.type, num: i.num, result: i.result !== undefined ? i.result : null }));
}
  ...
  try {
    switch (action) {
      ...
      case 'match-detail': {
        ...
        let recommends = [];
        try {
          recommends = await ensureRecommends(matchId);
        } catch (e) {}
