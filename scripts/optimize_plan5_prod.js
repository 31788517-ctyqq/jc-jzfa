...
function getOddsHistory(ds) {
  ...
  if (fs.existsSync(f)) { const r = JSON.parse(fs.readFileSync(f, 'utf8')); return r.odds || r; }
  return null;
}
function calcEffectiveOdds(odds, dir) {
  const sub = [];
  if (dir === '平、让平') { if (odds && odds.spf) sub.push(odds.spf.draw); if (odds && odds.rqspf) sub.push(odds.rqspf.draw); }
  else if (dir === '总进球-2、3球') { if (odds && odds.totalGoals) { if (odds.totalGoals['2']) sub.push(odds.totalGoals['2']); if (odds.totalGoals['3']) sub.push(odds.totalGoals['3']); } }
  if (sub.length === 0) return null; if (sub.length === 1) return sub[0];
...
function judgeByScore(dir, score, hcp) {
  ...
  if (dir === '平、让平') { const rq = hcp != null ? parseFloat(hcp) || 0 : 0; return (h === a) || (h + rq === a); }
  if (dir === '总进球-2、3球') { const t = h + a; return t === 2 || t === 3; }
  return null;
}

// ── 加载生产数据 ──
const dataFile = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'server', '_prod_data.json'), 'utf8'));
const mMap = dataFile.m || {}, rMap = dataFile.r || {};

function evaluatePlan5(params) {
  const { minExpertA, minExpertB, minOddsA, minOddsB, minProductOdds } = params;
  let invested = 0, income = 0, plans = 0, won = 0;

  const minDate = '2026-03-19', maxDate = '2026-06-06';
  const start = new Date(minDate), end = new Date(maxDate);
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    const ds = d.toISOString().slice(0, 10);
    const mList = [];
    Object.keys(mMap).forEach(k => { const m = mMap[k]; if (m && (m.date || '').slice(0, 10) === ds) mList.push(m); });
    if (mList.length < 6) continue;
    const histOdds = getOddsHistory(ds); if (!histOdds) continue;
    const mdMap = {};
    for (const m of mList) {
      const num = m.num || '', od = histOdds[num];
      const key = String(m.matchId);
      mdMap[m.matchId] = { match: m, recs: normalizeRecs(rMap['m_' + key] || rMap[key] || []), odds: od || null };
    }
    const ca = [], cb = [];
    for (const m of mList) {
      const md = mdMap[m.matchId]; if (!md || !md.odds) continue;
      let t = 0; for (const r of md.recs) { if (r.type === '平、让平') t += r.num || 0; }
      if (t < minExpertA) continue;
      const e = calcEffectiveOdds(md.odds, '平、让平'); if (!e || e < minOddsA) continue;
      ca.push({ match: m, count: t, odds: e });
    }
    for (const m of mList) {
      const md = mdMap[m.matchId]; if (!md || !md.odds || !md.odds.totalGoals) continue;
      const tg = md.odds.totalGoals; if (tg['2'] == null || tg['2'] <= 0 || tg['3'] == null || tg['3'] <= 0) continue;
      let t = 0; for (const r of md.recs) { if (r.type === '总进球-2、3球') t += r.num || 0; }
      if (t < minExpertB) continue;
      const e = calcEffectiveOdds(md.odds, '总进球-2、3球'); if (!e || e < minOddsB) continue;
      cb.push({ match: m, count: t, odds: e });
    }
    let best = null;
    for (const a of ca) for (const b of cb) {
      if (a.match.matchId === b.match.matchId) continue;
      const po = a.odds * b.odds; if (po < minProductOdds) continue;
      if (!best || po > best.pd) best = { a: a.match, b: b.match, pd: po, aO: a.odds, bO: b.odds };
    }
    if (!best) continue;
    const am = best.a, bm = best.b;
...
function* cartesian(grid) {
  ...
  function* recurse(idx, current) {
    ...
    for (const v of values[idx]) { current[keys[idx]] = v; yield* recurse(idx + 1, current); }
  }
  yield* recurse(0, {});
}

const results = [];
let total = 5 * 4 * 4 * 4 * 5, done = 0;
...
const current = results.find(r =>
  r.params.minExpertA === 2 && r.params.minExpertB === 0 &&
  r.params.minOddsA === 1.0 && r.params.minOddsB === 1.0 &&
