...
function post(path, body, referer) {
  return new Promise((resolve, reject) => {
    const req = https.request({
      method: 'POST', hostname: 'dstd.500.com', path, agent,
      headers: {
...
async function test() {
  ...
  for (const exp of testExperts) {
    ...
    const body = 'commresource=' + encodeURIComponent(JSON.stringify({ channel: 'mesport', platform: 'pc' })) +
      '&eid=' + exp.eid + '&pn=1&rn=10&articletype=';
    const text = await post('/transpondsanyol/api/meweb/expert/expert_history_articles', body,
      'https://dstd.500.com/zhuanjia/' + exp.eid);
    try {
      const j = JSON.parse(text);
      const arts = (j.status === '100' && j.data && j.data.articles) ? j.data.articles : [];
      if (arts.length === 0) { console.log('  → 无数据 ' + text.substring(0, 100)); continue; }
      ...
      console.log('  → ' + arts.length + '条, ' + times[times.length - 1] + ' ~ ' + times[0] +
        ', result:' + JSON.stringify(results) + ', pro:' + proCount + '场');
    } catch (e) {
