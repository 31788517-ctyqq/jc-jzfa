...
const mMap = raw.m || {}, rMap = raw.r || {};
const dateMap = {};
Object.keys(mMap).forEach(k => {
  const m = mMap[k], ds = (m.date || '').slice(0, 10);
...
allDates.forEach(ds => {
  ...
  if (fs.existsSync(f)) try { oddsCache[ds] = JSON.parse(fs.readFileSync(f, 'utf8')) } catch (e) {}
});

function getO(ds, n) { const od = oddsCache[ds]; return od && od.odds ? od.odds[n] || null : null }

// 场次A需要 spf.home, 场次B需要 rqspf.away
function hasWinOdds(o) { return o && o.spf && o.spf.home != null; }
function hasRqAway(o) { return o && o.rqspf && o.rqspf.away != null; }
...
function result(recs, d, score) {
  if (d === '胜') {
    // 比分优先
    if (score) {
      const p = String(score).split(':'), h = parseInt(p[0]), a = parseInt(p[1]);
...
function run(minDay, minOdds, minExpA, minExpB, minExpA2) {
  ...
  for (const ds of allDates) {
    const ml = dateMap[ds]; if (ml.length < minDay) continue;
    const mdM = {};
    for (const mm of ml) {
      ...
      const oo = getO(ds, mm.num || ''); let o = null;
    ...
    let bA = null, bAc = 0;
    for (const mm of ml) {
      const md = mdM[mm.matchId]; if (!md || !hasWinOdds(md.odds)) continue;
      let tA = 0; for (const r of md.recs) { if (r.type === '胜') tA += r.num || 0; }
      if (tA >= minExpA && tA > bAc) { bAc = tA; bA = mm; }
    }
    if (!bA) continue;
    // 场次B: 让负, 专家最多, 排除A
    let bB = null, bBc = 0;
    for (const mm of ml) {
      ...
      const md = mdM[mm.matchId]; if (!md || !hasRqAway(md.odds)) continue;
      let tB = 0; for (const r of md.recs) { if (r.type === '让负') tB += r.num || 0; }
      if (tB >= minExpB && tB > bBc) { bBc = tB; bB = mm; }
    }
    if (!bB) continue;
    // A专家门槛2: 胜专家数>=minExpA2 (可选第二道门槛)
    if (minExpA2 && bAc < minExpA2) continue;
    // 赔率
    const oA = eOdds(mdM[bA.matchId].odds, '胜'), oB = eOdds(mdM[bB.matchId].odds, '让负');
    ...
    t++;
    if (rA.won && rB.won) { w++; inc += Math.round(AMT * productOdds) - AMT; }
    else inc -= AMT;
  }
...
console.log('  日≥2, 合赔≥1.0, 无专家门槛: %d期 %d中 %.1f%% %+d\n', base.total, base.won, base.won / base.total * 100, base.income);
...
const results = [];
let done = 0, total = days.length * odds.length * expA.length * expB.length;
...
results.slice(0, 15).forEach((r, i) => {
  ...
  console.log('%3d   %3d   %4.1f   %3d   %4d   %4d   %3d   %5s%%  %+d',
    i + 1, r.day, r.odds, r.expA, r.expB, r.total, r.won, rate, r.income);
});
...
good.slice(0, 15).forEach((r, i) => {
  const rate = (r.won / r.total * 100).toFixed(1);
  console.log('  day≥%d  odds≥%.1f  胜≥%d  让负≥%d  →  %d期 %d中 %s%%  %+d',
    r.day, r.odds, r.expA, r.expB, r.total, r.won, rate, r.income);
});
