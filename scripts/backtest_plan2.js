...
function runBacktest(minDayMatches, minProdOdds) {
  const results = [];
  let totalP2 = 0, totalWon = 0, totalIncome = 0;
  let skipA = 0, skipB = 0, skipOdds = 0, skipDay = 0;

  for (const ds of allDates) {
    const mList = dateMap[ds];
    const dayMatchCount = mList.length;
    if (dayMatchCount < minDayMatches) { skipDay++; continue; }
    ...
    if (!bestMA) { skipA++; continue; }

    // ── 场次B：让负 方向专家最多的一场（排除场次A） ──
    let bestMB = null, bestCountB = 0;
    for (const mm of mList) {
    ...
    if (odA.length < 2 || odB.length < 1) { skipOdds++; continue; }

    const ddA = dutchOdds(odA);
    const ddB = odB[0];
    if (ddA <= 0 || ddB <= 0) { skipOdds++; continue; }

    const productOdds = ddA * ddB;
    if (productOdds < minProdOdds) { skipOdds++; continue; }
    ...
    results.push({
      date: ds,
      numA: bestMA.num || '', numB: bestMB.num || '',
      matchA: homeA + ' vs ' + visitA,
      matchB: homeB + ' vs ' + visitB,
      expertA: bestCountA, expertB: bestCountB,
      ddA: ddA.toFixed(2), ddB: ddB.toFixed(2),
      product: productOdds.toFixed(2),
      won, profit: won ? (prize - AMOUNT) : -AMOUNT,
    });
  }
...
for (const minDay of DAY_RANGE) {
  for (const minOdds of ODDS_RANGE) {
    ...
    grid.push({
      minDay, minOdds,
      total: r.totalP2, won: r.totalWon,
      rate: r.winRate, income: r.totalIncome,
      skipDay: r.skipDay, skipOdds: r.skipOdds,
    });
  }
}
...
for (const minDay of DAY_RANGE) {
  for (const minOdds of ODDS_RANGE) {
    ...
    console.log('  日≥' + String(minDay).padStart(2) + ' | 合赔≥' + String(minOdds.toFixed(1)).padStart(5) +
...
if (bestGrid) {
  ...
  if (bestR.results.length > 0) {
    console.log('\n  ── 明细 ──');
    console.log('  日期     场次A(总进球2,3球)            场次B(让负)                  专家A 合赔A 专家B 赔率B 合赔积  命中  盈亏');
    ...
    for (const r of bestR.results) {
      ...
      const h = r.won ? '✅' : '❌';
      const s = r.profit >= 0 ? '+' : '';
      console.log('  ' + d + '  ' + mA + mB + ea + ' ' + da + eb + ' ' + db + ' ' + prod + '  ' + h + '  ' + (s + r.profit).padStart(8));
    }
  }
}
