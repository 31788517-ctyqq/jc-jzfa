    ...
    const req = https.request(
      {
        method: 'POST',
        hostname: HOST,
        path: '/transpondsanyol/api/meweb/expert/expert_history_articles',
        agent,
        headers: {
          'User-Agent': 'Mozilla/5.0',
          'Content-Type': 'application/x-www-form-urlencoded',
          Referer: 'https://' + HOST + '/zhuanjia/' + EID,
          Origin: 'https://' + HOST,
        },
        timeout: 15000,
      },
...
function parseArticle(raw) {
  ...
  if (pro.length > 0) {
    for (const m of pro) {
      ...
      recs.push({
        matchnum: m.matchnum || '',
        home: m.home || (g ? g.home : ''),
        away: m.away || (g ? g.away : ''),
        league: m.ls || (g ? g.ls : ''),
        stime: m.stime || (g ? g.stime : ''),
        direction: dir,
        choice: m.choice || '',
        odds: m.pei || '',
        handicap: m.rangqiu || '',
        score: m.score || '',
        hit: m.result === 1 ? 'V' : m.result === 0 ? 'X' : '',
      });
    }
  }
  let ov = '待揭晓';
...
async function main() {
  ...
  while (pn <= 60) {
    ...
    let nc = 0,
      early = '';
    for (const a of arts) {
      if (a.aid && !seen.has(a.aid)) {
        seen.add(a.aid);
        all.push(parseArticle(a));
        nc++;
      }
  ...
  const out = {
    ...
    since: '2026-01-01',
    summary: {
      total_articles: flt.length,
      total_recommendations: tR,
      hit_count: hit.length,
      miss_count: mis.length,
      pending_count: pnd.length,
      hit_rate: rate,
  ...
  console.log(
    '\n已保存: ' +
      fpath +
      ' (' +
      (fs.statSync(fpath).size / 1024).toFixed(1) +
      'KB, 耗时' +
      out.summary.elapsed_seconds +
      's)',
  );
  ...
  hit.forEach((a) => {
    ...
    a.recommendations.forEach((r) => {
      console.log(
        '  ' + r.matchnum + ' ' + r.home + ' VS ' + r.away + ' | ' + r.direction + ' | ' + r.score + ' ' + r.hit,
      );
    });
  });
}
