/**
 * 实时比分同步脚本
 * 从 midou310.com 抓取比赛状态和比分，输出 live_scores.json
 * Node 10 兼容
 */
const https = require('https');
const fs = require('fs');
const path = require('path');

const CONFIG = {
  MIDOU_BASE: 'https://midou310.com/mdsj',
  MOBILE: process.env.MIDOU_MOBILE || '',
  PASSWORD: process.env.MIDOU_PASSWORD || '',
};

const OUTPUT = path.join(__dirname, 'live_scores.json');
const logger = require('./logger').child('score_sync');

function log(msg) {
  logger.info(msg);
}

function get(urlStr, params, headers) {
  return new Promise(function (resolve, reject) {
    let qs = '';
    if (params) {
      qs =
        '?' +
        Object.keys(params)
          .map(function (k) {
            return encodeURIComponent(k) + '=' + encodeURIComponent(params[k]);
          })
          .join('&');
    }
    const fullUrl = urlStr + qs;
    const urlObj = require('url').parse(fullUrl);

    const options = {
      hostname: urlObj.hostname,
      port: urlObj.port || 443,
      path: urlObj.path,
      method: 'GET',
      headers: Object.assign(
        {
          Accept: 'application/json, text/plain, */*',
          'User-Agent': 'Mozilla/5.0',
        },
        headers || {},
      ),
      rejectUnauthorized: false,
    };

    const req = https.request(options, function (res) {
      const chunks = [];
      res.on('data', function (c) {
        chunks.push(c);
      });
      res.on('end', function () {
        const buffer = Buffer.concat(chunks);
        let text = buffer.toString('utf-8');
        // Try GBK if UTF-8 fails
        try {
          JSON.parse(text);
        } catch (e) {
          try {
            text = require('iconv-lite').decode(buffer, 'gbk');
          } catch (e2) {}
        }
        try {
          resolve(JSON.parse(text));
        } catch (e) {
          reject(new Error('JSON parse fail: ' + text.slice(0, 200)));
        }
      });
    });
    req.on('error', reject);
    req.setTimeout(15000, function () {
      req.abort();
      reject(new Error('Timeout'));
    });
    req.end();
  });
}

async function sync() {
  try {
    // 1. Login
    log('开始同步比分...');
    const loginRes = await get(CONFIG.MIDOU_BASE + '/gduser/login.do', {
      mobile: CONFIG.MOBILE,
      password: CONFIG.PASSWORD,
    });
    if (loginRes.code !== 1) throw new Error('登录失败: ' + (loginRes.msg || 'unknown'));
    const token = loginRes.data.token;
    const today = (loginRes.data.today || new Date().toISOString()).slice(0, 10);

    // 2. Fetch today's matches
    const matchRes = await get(
      CONFIG.MIDOU_BASE + '/score/footballDataList.do',
      {
        time: Date.now(),
        order: 'status desc, start_datetime asc, data_id asc',
      },
      { Cookie: 'token=' + token },
    );

    if (matchRes.code !== 1) throw new Error('获取比赛失败: ' + (matchRes.msg || ''));
    const matches = (matchRes.data || []).map(function (m) {
      let md = '';
      if (m.bDate && typeof m.bDate === 'string' && m.bDate.length >= 10) md = m.bDate.slice(0, 10);
      if (!md) md = today;
      return {
        matchId: String(m.matchId || m.dataId || ''),
        num: m.num || '',
        homeName: m.homeName || '',
        visitName: m.visitName || '',
        leagueName: m.leagueName || '',
        startTime: m.startTime || m.start_datetime || '',
        matchStatus: m.matchStatus !== undefined ? m.matchStatus : 0,
        score: m.score || '',
        halfScore: m.halfScore || '',
        duration: m.duration || '',
        yellow: m.yellow || '',
        red: m.red || '',
        homeScore: m.homeScore !== undefined ? m.homeScore : -1,
        visitScore: m.visitScore !== undefined ? m.visitScore : -1,
        recommNum: m.recommNum || 0,
        date: md,
      };
    });

    // 3. 补充已结束/进行中比赛（多天历史数据）
    const seenIds = {};
    matches.forEach(function (m) {
      seenIds[m.matchId] = true;
    });
    try {
      const historyRes = await get(
        CONFIG.MIDOU_BASE + '/score/footballDataList.do',
        {
          time: Date.now() - 86400000,
          order: 'status desc, start_datetime asc, data_id asc',
        },
        { Cookie: 'token=' + token },
      );
      if (historyRes.code === 1 && historyRes.data) {
        (historyRes.data || []).forEach(function (m) {
          const mid = String(m.matchId || m.dataId || '');
          if (!seenIds[mid] && m.matchStatus >= 1) {
            seenIds[mid] = true;
            let hmd = '';
            if (m.bDate && typeof m.bDate === 'string' && m.bDate.length >= 10) hmd = m.bDate.slice(0, 10);
            if (!hmd) hmd = today;
            matches.push({
              matchId: mid,
              num: m.num || '',
              homeName: m.homeName || '',
              visitName: m.visitName || '',
              leagueName: m.leagueName || '',
              startTime: m.startTime || '',
              matchStatus: m.matchStatus,
              score: m.score || '',
              halfScore: m.halfScore || '',
              duration: m.duration || '',
              yellow: m.yellow || '',
              red: m.red || '',
              homeScore: -1,
              visitScore: -1,
              recommNum: m.recommNum || 0,
              date: hmd,
            });
          }
        });
      }
    } catch (e) {
      log('补充历史比赛: ' + e.message);
    }

    // 4. Write output
    const output = { date: today, matches: matches, updated: new Date().toISOString() };
    fs.writeFileSync(OUTPUT, JSON.stringify(output));

    const liveCount = matches.filter(function (m) {
      return m.matchStatus === 1;
    }).length;
    const finishedCount = matches.filter(function (m) {
      return m.matchStatus === 2;
    }).length;
    log('同步完成: ' + matches.length + '场, 进行中:' + liveCount + ', 已结束:' + finishedCount);
  } catch (e) {
    log('同步失败: ' + e.message);
    process.exit(1);
  }
}

// Load .env if available
try {
  const envFile = fs.readFileSync(path.join(__dirname, '.env'), 'utf-8');
  envFile.split('\n').forEach(function (line) {
    const parts = line.trim().split('=');
    if (parts.length === 2 && parts[0] && !parts[0].startsWith('#')) {
      process.env[parts[0]] = parts[1];
    }
  });
  CONFIG.MOBILE = process.env.MIDOU_MOBILE || CONFIG.MOBILE;
  CONFIG.PASSWORD = process.env.MIDOU_PASSWORD || CONFIG.PASSWORD;
} catch (e) {}

if (!CONFIG.MOBILE || !CONFIG.PASSWORD) {
  log('缺少 MIDOU_MOBILE/MIDOU_PASSWORD 环境变量');
  process.exit(1);
}

sync();
