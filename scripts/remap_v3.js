...
const backupRows = backupDB
  .prepare('SELECT matchId, num, homeName, visitName, date FROM matches WHERE date >= ? AND date <= ?')
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            resolve(JSON.parse(Buffer.concat(d).toString()));
          } catch (e) {
            resolve({ code: -1 });
          }
        });
...
async function main() {
  ...
  for (const dateStr of dates) {
    ...
    for (const mm of midouMatches) {
      ...
      if (!found) {
        for (const [oldNum, entry] of Object.entries(odds)) {
          if (dbUsed.has(oldNum)) continue;
          if (
            cleanName(entry.homeName || '') === cleanName(mm.homeName || '') &&
    ...
    const dayOfWeek =
      dateStr.slice(8) === '19' ? '周四' : (odds[maxNum] || { num: '*' }).num.slice(0, 2) || getDay(dateStr);
