  ...
  return new Promise((r, e) => {
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            r(JSON.parse(Buffer.concat(d).toString()));
          } catch (e) {
            r({ code: -1 });
          }
        });
...
async function main() {
  ...
  for (const d of testDates) {
    ...
    for (const mm of sorted) {
      const dbm = db[mm.num];
      if (dbm) {
        const dbHome = cleanName(dbm.homeName || ''),
          dbAway = cleanName(dbm.visitName || '');
        const mdHome = cleanName(mm.homeName || ''),
          mdAway = cleanName(mm.visitName || '');
