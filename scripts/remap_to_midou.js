      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ code: -1, msg: String(e).slice(0, 200) });
          }
        });
      },
    );
...
async function main() {
  ...
  for (const dateStr of dates) {
    ...
    console.log(
      `  ${dateStr}: midou${mr.data.length} | DB${Object.keys(odds).length} | remap:${remapped} unchange:${unchanged} nf:${notfound}${extra}`,
    );
  ...
  console.log(`\n=== Remap Complete ===`);
  console.log(
    `Total remapped: ${totalRemapped} | unchanged: ${totalUnchanged} | not found: ${totalNotFound} | skipped: ${totalSkipped}`,
  );
}
