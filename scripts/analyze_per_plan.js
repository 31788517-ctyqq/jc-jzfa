...
function printPlanStats(planStats, title) {
  ...
  let totalPlans = 0, totalIncome = 0;
  ...
  for (const key of order) {
    ...
    console.log('  ' + colorTag + ' ' + name.padEnd(8) +
  ...
  console.log('  📊 合计     ' + String(totalPlans).padStart(6) +
    '                        ' + tSign + totalIncome);
  console.log('');
...
for (const dayT of P6_DAY_THRESHOLDS) {
  for (const expT of P6_EXPERT_THRESHOLDS) {
    comboDone++;
    process.stdout.write('\r  Scanning ' + comboDone + '/' + totalCombos + ' (day>=' + dayT + ', expert>=' + expT + ')...');
    const stats = analyzeAllSchemes(dayT, expT);
    const p6 = stats.plan_6;
    // 同时统计全部方案
    let allTotal = 0, allIncome = 0;
    ...
    scanResults.push({
      dayT, expT,
      p6_total: p6.total, p6_won: p6.won, p6_income: p6.income,
      all_total: allTotal, all_income: allIncome,
    });
  }
}
...
if (p6Profitable.length > 0) {
  ...
  console.log('  ★ 预期结果: ' + best.p6_total + ' 个方案六, 命中率 ' +
    (best.p6_total > 0 ? (best.p6_won / best.p6_total * 100).toFixed(1) : '0.0') +
    '%, 盈利 +' + best.p6_income + ' 元');
  console.log('  ★ 全部方案合计: ' + best.all_total + ' 个, 总盈利 ' +
    (best.all_income >= 0 ? '+' : '') + best.all_income + ' 元');
  
  // 对比基线
  const baselineP6 = baseline.plan_6;
  console.log('');
  console.log('  📊 对比基线（当前 day>=4, expert>=1）:');
  console.log('     基线: ' + baselineP6.total + ' 方案六, 盈利 ' +
    (baselineP6.income >= 0 ? '+' : '') + baselineP6.income + ' 元');
  console.log('     优化: ' + best.p6_total + ' 方案六, 盈利 +' + best.p6_income + ' 元');
  console.log('     变化: ' + (best.p6_total - baselineP6.total >= 0 ? '+' : '') +
    (best.p6_total - baselineP6.total) + ' 方案, ' +
    (best.p6_income - baselineP6.income >= 0 ? '+' : '') +
    (best.p6_income - baselineP6.income) + ' 元');
  
  // 显示与最佳方案数接近的前5个选择
  console.log('');
  console.log('  🔝 方案数最多的盈利组合 TOP 5:');
  p6Profitable.slice(0, 5).forEach((r, i) => {
    console.log('     ' + (i + 1) + '. day>=' + r.dayT + ', exp>=' + r.expT +
      ' → 方案六=' + r.p6_total + ', 盈利=+' + r.p6_income + '元');
  });
} else {
  console.log('');
  ...
  console.log('  亏损最少: day>=' + bestLoss.dayT + ', exp>=' + bestLoss.expT +
    ' → ' + bestLoss.p6_total + ' 个, ' + bestLoss.p6_income + ' 元');
}
