 ...
 * 世界杯方案定制回测 — 生产端运行
 * 规则: 各方案独立参数 + 独立盈利目标
 * 运行: cd /root/server && node backtest_wc_config.cjs
 */
var fs = require('fs');
...
var SCHEMES = {
  ...
  '3': { name: '方案三', profitPct: 0.30, topN: 5, minRecsA: 50 },
  '4': { name: '方案四', profitPct: 0.30, topN: 6, minRecsB: 20 },
  '5': { name: '方案五', profitPct: 0, topN: 7, minOddsA: 1.3, minOddsB: 1.3, minProd: 1.5 },
  '6': { name: '方案六', profitPct: 0.50, topN: 6 },
  '7': { name: '方案七', profitPct: 0.10, topN: 0, minRecsA: 10 },
  '8': { name: '方案八', profitPct: 0.05, topN: 2, minRecsA: 50 },
};
...
function findBestMatchForDirection(dirs, excludeIds, minCount, mList, mdm) {
  var best = null, bestCount = 0;
  for (var i = 0; i < mList.length; i++) {
    var m = mList[i];
    if (excludeIds && excludeIds.indexOf(m.matchId) >= 0) continue;
    var md = mdm[m.matchId]; if (!md || !md.odds) continue;
    var total = 0;
    for (var j = 0; j < md.recs.length; j++) { if (dirs.indexOf(md.recs[j].type) >= 0) total += md.recs[j].num || 0; }
    if (total > bestCount) { bestCount = total; best = m; }
  }
...
function extractSubOdds(oddsObj, direction) {
  var vals = []; if (!oddsObj) return vals;
  ...
  if (direction.indexOf('、') >= 0 || direction.indexOf(',') >= 0) {
    ...
    parts.forEach(function (pd) { pd = pd.trim();
...
function getMatchOdds(m) {
  var md = m._md; if (!md || !md.odds) return null;
  var o = md.odds;
  return {
...
function buildMatchObj(m, direction) {
  ...
  subDirs.forEach(function (subDir) {
    ...
    if (!found) for (var rj = 0; rj < recs.length; rj++) {
  ...
  matchedRecs.forEach(function (r) { if (r.result === 1) anyWon = true; else if (r.result === 0) anyLose = true; else anyUnknown = true; });
  if (!anyUnknown && matchedRecs.length > 0) { isMatchWon = anyWon; isMatchLose = !anyWon && anyLose; }
  if (isMatchWon === null && isMatchLose === null && m.matchStatus >= 1 && m.score) {
    var mOdds = getMatchOdds(m); var hcp = mOdds && mOdds.rqspf ? mOdds.rqspf.handicap : null;
    function j(d, s, h) {
      if (d.indexOf('、') >= 0) { var p = d.split(/[、,]/); for (var pi = 0; pi < p.length; pi++) { if (j(p[pi].trim(), s, h)) return true; } return false; }
      var pp = String(s).replace(/[-:]/g, ':').split(':'); var hh = parseInt(pp[0]), aa = parseInt(pp[1]); if (isNaN(hh) || isNaN(aa)) return null;
      ...
      var gm = d.match(/总进球-(\d+)/); if (gm) return hh + aa === parseInt(gm[1]); return null;
    }
    var sr = j(direction, m.score, hcp); if (sr !== null) { isMatchWon = sr; isMatchLose = !sr; }
  }
  return {
    matchId: m.matchId, homeName: m.homeName, visitName: m.visitName, leagueName: m.leagueName,
    matchNum: m.num || '', startTime: m.startTime || '', matchStatus: m.matchStatus || 0, direction: direction,
    expertCount: expertCount, isMatchWon: isMatchWon, isMatchLose: isMatchLose, subResults: subResults,
...
function make2x1Plan(id, sName, mA, dirA, mB, dirB, ds) {
  ...
  var productOdds = e1 && e2 ? e1 * e2 : 0, maxPrize = e1 && e2 ? Math.round(AMOUNT * productOdds) : 0;
  var pr = computePlanResult([aObj, bObj]);
  return { planId: 'plan_' + ds + '_' + id, name: 'plan_' + id, planName: sName, matches: [aObj, bObj],
    amount: AMOUNT, playType: '混合投注', matchCount: 2, passType: '2串1', betCount: 250, ticketCount: 10, multiplier: 25,
    maxPrize: maxPrize, winningPrize: pr.isPlanWon === true ? maxPrize : 0,
    isPlanWon: pr.isPlanWon, isPlanLose: pr.isPlanLose, productOdds: productOdds };
}
...
function makeSinglePlan(id, sName, m, direction, ds) {
  ...
  var pr = computePlanResult([mObj]);
  if (direction === '胜' && mObj.odds && mObj.odds.spf) maxPrize = Math.round(AMOUNT * mObj.odds.spf.home);
  return { planId: 'plan_' + ds + '_' + id, name: 'plan_' + id, planName: sName, matches: [mObj],
    amount: AMOUNT, playType: '单关', matchCount: 1, passType: '单关', betCount: 250, ticketCount: 10, multiplier: 25,
    maxPrize: maxPrize, winningPrize: pr.isPlanWon === true ? maxPrize : 0,
    isPlanWon: pr.isPlanWon, isPlanLose: pr.isPlanLose, productOdds: maxPrize > 0 ? maxPrize / AMOUNT : 0 };
}

function meetsSchemeProfit(plan, pct) { return plan.maxPrize >= AMOUNT * (1 + pct); }

// ═══ 主回测 ═══
var dates = new Set();
Object.keys(mMap).forEach(function (k) { var m = mMap[k]; if (m && m.date) { var d = m.date.slice(0, 10); if (d >= START && d <= END) dates.add(d); } });
var sortedDates = Array.from(dates).sort();
var totalPlans = 0, totalWon = 0, totalProfit = 0;
var planTypeStats = {};

console.log('══════════════════════════════════════════════════════════════');
console.log('  世界杯方案定制回测 (6月, 仅真实世界杯比赛, 生产端)');
console.log('══════════════════════════════════════════════════════════════');
...
sortedDates.forEach(function (ds) {
  ...
  mList.forEach(function (m) {
    ...
    m._md = { match: m, recs: normalizeRecs(raw), odds: od ? { spf: od.spf || null, rqspf: od.rqspf || null, totalGoals: od.totalGoals || null, halfFull: od.halfFull || null, isSingleGame: od.isSingleGame || false } : null };
    mdm[m.matchId] = m._md;
  });

  var ranking = [];
  mList.forEach(function (m) { var md = m._md; if (!md || !md.recs) return; md.recs.forEach(function (r) { if (!r.type || !r.num) return; ranking.push({ matchId: m.matchId, direction: r.type, count: r.num }); }); });
  ...
  if (hasRecs && matchCount >= 4) {
    var cA = [], cB = [];
    mList.forEach(function (ma) { var md = ma._md; if (!md || !md.odds) return; var aO = buildMatchObj(ma, '平、让平'); var eA = calcEffectiveOdds('平、让平', aO.odds); if (!eA || eA < SCHEMES['5'].minOddsA) return; var tA = 0; md.recs.forEach(function (r) { if (r.type === '平' || r.type === '让平') tA += r.num || 0; }); if (tA > 0 && isDirectionTopN(ma.matchId, ['平', '让平'], SCHEMES['5'].topN, ranking)) cA.push({ m: ma, odds: eA }); });
    mList.forEach(function (mb) { var md = mb._md; if (!md || !md.odds || !md.odds.totalGoals) return; var tg = md.odds.totalGoals; if (tg['2'] == null || tg['2'] <= 0 || tg['3'] == null || tg['3'] <= 0) return; var bO = buildMatchObj(mb, '总进球-2、3球'); var eB = calcEffectiveOdds('总进球-2、3球', bO.odds); if (!eB || eB <= SCHEMES['5'].minOddsB) return; var tB = 0; md.recs.forEach(function (r) { if (r.type === '总进球-2、3球') tB += r.num || 0; }); if (tB > 0 && isDirectionTopN(mb.matchId, ['总进球-2、3球'], SCHEMES['5'].topN, ranking)) cB.push({ m: mb, odds: eB }); });
    var bp5 = null;
    cA.forEach(function (ca) { cB.forEach(function (cb) { if (ca.m.matchId === cb.m.matchId) return; var prod = ca.odds * cb.odds; var mp = SCHEMES['5'].minProd || 1.8; if (prod < mp) return; if (!bp5 || prod > bp5.productOdds) bp5 = { a: ca.m, b: cb.m, productOdds: prod }; }); });
    if (bp5) { var p5 = make2x1Plan('5', SCHEMES['5'].name, bp5.a, '平、让平', bp5.b, '总进球-2、3球', ds); if (meetsSchemeProfit(p5, SCHEMES['5'].profitPct)) plans.push(p5); }
  }
  // 方案六
  if (hasRecs && matchCount >= 2) {
    var bM6 = null, bC6 = 0;
    mList.forEach(function (m) { var md = m._md; if (!md || !md.odds || !md.odds.totalGoals) return; if (md.odds.totalGoals['2'] == null || md.odds.totalGoals['3'] == null) return; var t6 = 0; md.recs.forEach(function (r) { if (r.type === '总进球-2、3球') t6 += r.num || 0; }); if (t6 > bC6) { bC6 = t6; bM6 = m; } });
    if (bM6 && bC6 > 0 && isDirectionTopN(bM6.matchId, ['总进球-2、3球'], SCHEMES['6'].topN, ranking)) { var p6 = makeSinglePlan('6', SCHEMES['6'].name, bM6, '总进球-2、3球', ds); if (meetsSchemeProfit(p6, SCHEMES['6'].profitPct)) plans.push(p6); }
  }
  // 方案七
  if (hasRecs) {
    var sM = mList.filter(function (m) { var md = m._md; return md && md.odds && md.odds.isSingleGame === true; });
    if (sM.length > 0) { var bM7 = null, bD7 = '', bC7 = 0; sM.forEach(function (sm) { sm._md.recs.forEach(function (r) { if ((r.type === '胜平' || r.type === '平负') && r.num > bC7) { bC7 = r.num; bM7 = sm; bD7 = r.type; } }); }); if (bM7 && bD7 && bC7 >= SCHEMES['7'].minRecsA) { var p7 = makeSinglePlan('7', SCHEMES['7'].name, bM7, bD7, ds); if (meetsSchemeProfit(p7, SCHEMES['7'].profitPct)) plans.push(p7); } }
  }
  // 方案八
  if (hasRecs) {
    var sM8 = mList.filter(function (m) { var md = m._md; return md && md.odds && md.odds.isSingleGame === true; });
    if (sM8.length > 0) { var bM8 = null, bC8 = 0; sM8.forEach(function (sm) { sm._md.recs.forEach(function (r) { if (r.type === '胜' && r.num > bC8) { bC8 = r.num; bM8 = sm; } }); }); if (bM8 && bC8 >= SCHEMES['8'].minRecsA && isDirectionTopN(bM8.matchId, ['胜'], SCHEMES['8'].topN, ranking)) { var p8 = makeSinglePlan('8', SCHEMES['8'].name, bM8, '胜', ds); if (meetsSchemeProfit(p8, SCHEMES['8'].profitPct)) plans.push(p8); } }
  }
  ...
  plans.forEach(function (p) {
    ...
    var r = '?';
    if (p.isPlanWon === true) { totalWon++; planTypeStats[p.name].won++; var pf = (p.winningPrize || 0) - AMOUNT; dayProfit += pf; planTypeStats[p.name].profit += pf; r = '✓'; }
    else if (p.isPlanLose === true) { dayProfit -= AMOUNT; planTypeStats[p.name].profit -= AMOUNT; r = '✗'; }
    dayNames.push(p.planName + '(' + r + p.maxPrize + ')');
  });
  if (dayNames.length > 0) { totalPlans += dayNames.length; totalProfit += dayProfit; console.log(ds + '\t' + matchCount + '\t' + dayNames.length + '\t' + dayProfit + '\t' + totalProfit + '\t' + dayNames.join(' ')); }
});
...
console.log('\n── 按方案类型 ──');
var planNames = {}; for (var pk in SCHEMES) planNames['plan_' + pk] = SCHEMES[pk].name;
Object.keys(planTypeStats).sort().forEach(function (k) { var s = planTypeStats[k]; console.log((planNames[k] || k).padEnd(8), '总数=' + String(s.total).padEnd(4), '命中=' + String(s.won).padEnd(4), '命中率=' + (s.total > 0 ? ((s.won / s.total) * 100).toFixed(1) + '%' : '-').padEnd(8), '盈利=' + s.profit); });
