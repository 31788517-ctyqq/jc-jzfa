...
const totals = { days: 0, matches: 0, spf: 0, spfMiss: 0, rqspf: 0, rqspfMiss: 0, jqs: 0, jqsMiss: 0 };
const issues = [];
...
console.log(
  `SPF (胜平负):     ${totals.spf}/${totals.matches} 有效 (${((totals.spf / totals.matches) * 100).toFixed(1)}%)，缺失 ${totals.spfMiss} 场`,
);
console.log(
  `RQSPF (让球胜平负): ${totals.rqspf}/${totals.matches} 有效 (${((totals.rqspf / totals.matches) * 100).toFixed(1)}%)，缺失 ${totals.rqspfMiss} 场`,
);
console.log(
  `JQS (进球数):      ${totals.jqs}/${totals.matches} 有效 (${((totals.jqs / totals.matches) * 100).toFixed(1)}%)，缺失 ${totals.jqsMiss} 场`,
);
...
console.log('\n=== 日期连续性 ===');
const expectedDates = [];
const cur = new Date(DATE_START + 'T00:00:00+08:00');
