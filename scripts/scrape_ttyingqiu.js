    ...
    https
      .get(
        ...
        (res) => {
          ...
          if (sc) {
            globalCookies = (Array.isArray(sc) ? sc : [sc]).map((c) => c.split(';')[0]).join('; ');
          }
          res.resume();
          resolve();
        },
...
const CONFIG = {
  ...
  BUILTIN_EXPERTS: [{ id: '212520', name: '杨子墨', raceType: 1 }],
...
async function discoverExperts() {
  ...
  const target = 300;
  console.log(
    `探测范围: ${CONFIG.EXPERT_ID_RANGE_START}~${CONFIG.EXPERT_ID_RANGE_END}, 步长: ${step}, 并发: ${concurrent}`,
  );
  ...
  for (let i = 0; i < ids.length; i += concurrent) {
    ...
    const results = await Promise.allSettled(batch.map((id) => probeExpert(id, CONFIG.RACE_TYPE_FOOTBALL)));
...
function parseDetailPage(html, interpId) {
  ...
  while ((m = matchRegex.exec(text))) {
    ...
    matchBlocks.push({
      ...
      pick: pick,
      spf_odds: oddsIn
        ? { win: parseFloat(oddsIn[1]), draw: parseFloat(oddsIn[2]), lose: parseFloat(oddsIn[3]) }
        : null,
    });
  }
  ...
  const analysis =
    analysisStart >= 0 ? text.substring(analysisStart, analysisStart + 800).trim() : text.substring(0, 500);
...
async function fetchDetail(interpId) {
  const html = await httpGet(`/interpretation/detail/${interpId}`, `https://${HOST}/expert/home/`);
...
function saveInterpretations(results, errors, experts) {
  ...
  const completed = flatList.filter((i) => i.matches && i.matches.some((m) => m.status === '已完场'));
