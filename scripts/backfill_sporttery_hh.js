...
async function fetchMatch(matchId) {
  ...
  try {
    ...
    return {
      ...
      halfFull: {
        hh: first.hh || null, hd: first.hd || null, ha: first.ha || null,
        dh: first.dh || null, dd: first.dd || null, da: first.da || null,
        ah: first.ah || null, ad: first.ad || null, aa: first.aa || null,
      },
...
async function main() {
  ...
  if (fs.existsSync(CACHE_FILE)) {
    try { cache = JSON.parse(fs.readFileSync(CACHE_FILE, 'utf8')); console.log(`[CACHE] 已有 ${Object.keys(cache).length} 条缓存`); } 
    catch(e) {}
  }
  ...
  for (let i = 0; i < idList.length; i += BATCH_SIZE) {
    ...
    if ((i / BATCH_SIZE) % 5 === 0 || i + BATCH_SIZE >= idList.length) {
      process.stdout.write(`\r  进度: ${Math.min(i + BATCH_SIZE, idList.length)}/${idList.length} | 新抓:${newFetch} 缓存:${cachedUse}   `);
    }
  ...
  let updatedMatches = 0, updatedFiles = 0;
