...
function renderAIContent(content, homeTeam, awayTeam) {
  ...
  if (hf || af || hasHistory || hasInj || hasStateCon) {
    ...
    if (hasInj) { html += '<div class="ai-injury-title">伤停对比</div>'; injTable.rows.forEach(function(row) { if (row.length < 3) return; html += '<div class="ai-injury-row"><div class="ai-injury-head"><span class="ai-injury-team">' + esc(row[0]) + '</span><span class="ai-injury-status ' + (row[2].indexOf('缺') >= 0 || row[2].indexOf('停') >= 0 ? 'out' : 'doubt') + '">' + esc(row[2]) + '</span>' + (row[3] ? '<span class="ai-injury-effect ' + (row[3].indexOf('高') >= 0 ? 'high' : 'low') + '">' + esc(row[3]) + '</span>' : '') + '</div><div class="ai-injury-detail">' + esc(row[1]) + '</div></div>'; }); }
