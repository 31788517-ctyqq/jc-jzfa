...
function renderProfitChartWc(wcDates, wcProfits) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    const dotColor = values[i] >= 0 ? '#ff5858' : '#22c878';
    dotHtml +=
  ...
  for (let g = 0; g <= ySteps; g++) {
    const gy = toY(yMin + yStep * g);
    const isZero = (yMin + yStep * g) === 0;
    gridLines += '<line x1="' + padX + '" y1="' + gy.toFixed(1) + '" x2="' + (svgW - padX) + '" y2="' + gy.toFixed(1) + '" stroke="' + (isZero ? '#475569' : '#dce2e5') + '" stroke-width="' + (isZero ? '1' : '0.5') + '" stroke-dasharray="' + (isZero ? '4,3' : '3,4') + '"/>';
  }
  ...
  // Y axis labels（从 yMax 到 yMin，正数在上负数在下）
  let yLabels = '';
  for (let j = 0; j <= ySteps; j++) {
    const val = Math.round(yMax - yStep * j);
    yLabels += '<span>' + (val >= 0 ? '+' : '') + val + '</span>';
  }
  const yAxisEl = document.getElementById('wcProfitYaxis');
  if (yAxisEl) yAxisEl.innerHTML = yLabels;

  // X axis labels（与专家图表一致：flex evenly-spaced，按 step 提取）
  const xAxisEl = document.getElementById('wcProfitXaxis');
  if (xAxisEl) {
    let dHtml = '';
    const showStep = n > 5 ? 2 : 1;
    for (var i = 0; i < n; i += showStep) {
      dHtml += '<span>' + (labels[i] || '') + '</span>';
    }
    // 确保最后一个日期始终显示
    if (showStep > 1 && (n - 1) % showStep !== 0) {
      dHtml += '<span>' + (labels[n - 1] || '') + '</span>';
    }
    xAxisEl.innerHTML = dHtml;
  }
  ...
  if (totalEl) {
    ...
    totalEl.style.color = total >= 0 ? '#ff5858' : '#22c878';
  }
