...
window.loadRefList = async function () {
  ...
  if (wList.length === 0) {
    ...
    let rows = wList.map(w => `<tr>
      ...
      <td><span class="admin-status admin-status-${w.status}">${w.status}</span></td>
      <td>
...
window.processWithdraw = async function (id, newStatus) {
  ...
  const res = await api('/api', {
    action: 'admin-referral-withdraw-process', token: true,
    withdrawalId: id, newStatus, remark,
  });
  if (res.code === 0) {
    loadRefList();
  } else {
