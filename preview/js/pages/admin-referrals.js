  ...
  try {
    ...
    if (list.length === 0) {
      ...
      const rows = list
        .map(
          (r) =>
            ...
            (r.invitee_name || '--') +
            '</td><td>¥' +
            ((r.payment_amount || 0) / 100).toFixed(2) +
            '</td><td>' +
            r.commission_rate +
            '%</td><td>¥' +
            ((r.commission_amount || 0) / 100).toFixed(2) +
            '</td><td><span class="admin-status admin-status-' +
            r.status +
            '">' +
      ...
      document.getElementById('refList').innerHTML =
        '<table class="admin-table"><thead><tr><th>邀请人</th><th>被邀请人</th><th>实付</th><th>比例</th><th>返利</th><th>状态</th><th>时间</th></tr></thead><tbody>' +
        rows +
        '</tbody></table>';
    }
      ...
      const rows = wList
        .map(
          (w) =>
            ...
            w.user_id +
            ')</td><td>¥' +
            ((w.amount || 0) / 100).toFixed(2) +
            '</td><td>' +
            (METHOD_CN[w.payment_method] || w.payment_method || '--') +
            '</td><td>' +
            (w.account_holder || '') +
            '</td><td>' +
            (w.payment_account || '').slice(-4) +
            '</td><td><span class="admin-status admin-status-' +
            w.status +
            '">' +
            (STATUS_CN[w.status] || w.status || '--') +
            '</span></td><td><button onclick="processWithdraw(' +
            w.id +
            ', \'completed\')" class="admin-btn-sm admin-btn-ok">✓ 打款</button> <button onclick="processWithdraw(' +
            w.id +
            ', \'rejected\')" class="admin-btn-sm admin-btn-no">✕ 驳回</button></td></tr>',
        )
        .join('');
      document.getElementById('withdrawList').innerHTML =
        '<table class="admin-table"><thead><tr><th>用户</th><th>金额</th><th>方式</th><th>收款人</th><th>尾号</th><th>状态</th><th>操作</th></tr></thead><tbody>' +
        rows +
        '</tbody></table>';
    }
  } catch (e) {
