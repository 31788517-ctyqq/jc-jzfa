...
export function loadQuantRank() {
  ...
  var cacheKey = 'quant-rank:' + (quantDate || 'latest');
  ...
  api('ranking-list', params)
    .then(function (rankData) {
      ...
      var gsAllPromise = api('gongshoudao-all', params).catch(function () {
        return {};
      });
      var hotPromise = api('quant-hot', params).catch(function () {
        return null;
      });
...
function renderCrossValue(item) {
  ...
  var spfLine = '',
    hcpLine = '';
  var spfW = item.crossSpfWin,
    spfD = item.crossSpfDraw,
    spfL = item.crossSpfLose;
  if (spfW !== '-' && spfW !== undefined) {
    spfLine =
      '胜' + Number(spfW).toFixed(0) + '% 平' + Number(spfD).toFixed(0) + '% 负' + Number(spfL).toFixed(0) + '% (让0)';
  }
  var hcpW = item.crossHcpWin,
    hcpD = item.crossHcpDraw,
    hcpL = item.crossHcpLose;
  var rqVal = item.crossRq !== undefined ? item.crossRq : 0;
  if (hcpW !== '-' && hcpW !== undefined) {
    hcpLine =
      '让胜' +
      Number(hcpW).toFixed(0) +
      '% 让平' +
      Number(hcpD).toFixed(0) +
      '% 让负' +
      Number(hcpL).toFixed(0) +
      '% (让' +
      rqVal +
      ')';
  }
  ...
  if (spfLine || hcpLine) {
    detail =
      '<span style="display:block;font-size:9px;color:#64748B;line-height:1.2;margin-top:1px">' +
      (spfLine ? spfLine : '') +
      (spfLine && hcpLine ? '<br>' : '') +
      (hcpLine ? hcpLine : '') +
      '</span>';
  ...
  return (
    '<span class="q-col-cross"><span class="q-cell-num ' +
    cls +
    '">' +
    (n >= 0 ? '+' : '') +
    n.toFixed(2) +
...
function _doRenderChart(container) {
  ...
  seriesDefs.forEach(function (def, i) {
    ...
    // 提取数据（带标签信息给 tooltip）
    var rawData = filtered.map(function (item, j) {
      var raw;
      ...
      var val = parseFloat(raw.toFixed(def.fmt));
      var tags = computeTags(item);
      var tagStr = tags
        .map(function (t) {
          return t.e + t.t;
        ...
        .join(' ');
      return { value: val, _tags: tagStr, _name: names[j] };
    });
  ...
  chartInstance.setOption({
    tooltip: {
      ...
      formatter: function (params) {
        ...
        var p = params[0];
        var tags = (p.data && p.data._tags) ? '<br/><span style="color:#94A3B8;font-size:10px">' + p.data._tags + '</span>' : '';
        return '<b>' + (p.name || (p.data && p.data._name) || '') + '</b><br/>' + p.marker + ' ' + p.seriesName + '：<b>' + p.value + '</b>' + tags;
      },
    },
    legend: {
      data: legendData,
