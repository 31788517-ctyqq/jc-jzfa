...
function normalizePlan(data) {
  const incoming = data && data.plan_code ? data : readStoredPlan() || {};
  const code = incoming.plan_code || 'yearly';
  const meta = PLAN_META[code] || PLAN_META.yearly;
  return {
    plan_code: code,
...
function formatPaymentError(message) {
  const map = {
    ...
    ALIPAY_ISV_PERMISSION_DENIED: '支付宝商户签约权限尚未生效，请联系管理员检查签约状态。',
  };
...
export async function loadPayment(container, data) {
  ...
  const html =
    ...
    '<div class="payment-actions payment-actions-top">' +
    '<button class="payment-submit-btn" id="payBtn" type="button" style="background:linear-gradient(90deg,#ff9a76,#ff7f66 45%,#ff6b6b);color:#fff;border:1px solid rgba(255,255,255,.28);box-shadow:0 16px 32px rgba(255,107,107,.32);" onclick="submitPayment(\'' +
    plan.plan_code +
    "'," +
    Number(plan.price || 0) +
    ')">确认支付 ' +
    ...
    '</div>' +
    '</div>' +
    '<div id="paymentStatus" class="payment-status-box" style="display:none"><div class="loading-spinner"></div><p id="paymentStatusText" class="payment-status-text">正在生成订单...</p></div>' +
    '<div class="member-cta-row payment-bottom-actions">' +
    '<button class="member-secondary-btn" type="button" onclick="window.navigateTo(\'profile\')">返回用户中心</button>' +
...
window.submitPayment = async function (planCode, price) {
  ...
  try {
    ...
    const order = await api('payment-create-order', body, 0);
